...
public class InventoryReceiptBean implements InventoryReceiptRemote,
    ...
    public InventoryReceiptViewDO add(InventoryReceiptViewDO data) throws Exception {
        ...
        } else if ("Y".equals(item.getIsBulk()) &&
                   "Y".equals(data.getAddToExistingLocation())) {
            //
        ...
        do {
            if (count != 0) {
                ...
                entityLocation = manager.find(InventoryLocation.class,
                                              locationData.getId());
                entityLocation.setQuantityOnhand(entityLocation.getQuantityOnhand() +
                                                 quantity);
            }
    ...
    public InventoryReceiptViewDO update(InventoryReceiptViewDO data) throws Exception {
        ...
        if ("Y".equals(itemData.getIsSerialMaintained())) {
            ...
            // the system at the inventory location associated with this
            // inventory
            ...
            //
            oldEntityLocation.setQuantityOnhand(oldEntityLocation.getQuantityOnhand() +
                                                dQ);
            ...
            // if current quantity is zero it means that we didn't receive any
            // items and thus we need to delete the record that acts as the
            // receipt for the items as well as the xput record associated with
            // it
            //
            if (currQ == 0) {
                manager.remove(entity);
        ...
        if ( ( !oldEntityLocation.getId().equals(locationData.getId())) &&
            ( !oldEntityLocation.getStorageLocationId()
                                .equals(locationData.getStorageLocationId()))) {
            if ("Y".equals(itemData.getIsSerialMaintained())) {
    ...
    public InventoryReceiptViewDO fetchForUpdate(Integer id) throws Exception {
        lock.lock(Constants.table().INVENTORY_RECEIPT, id);
        return fetchById(id);
    }

    public InventoryReceiptViewDO abortUpdate(Integer id) throws Exception {
        lock.unlock(Constants.table().INVENTORY_RECEIPT, id);
        return fetchById(id);
    }
    ...
    public void validate(InventoryReceiptViewDO data) throws Exception {
        ...
        //
        // we do this here in order to make sure we don't try to validate a
        // record
        // that the user had no intentions of updating on the screen but was not
        // able
        // to remove because it represented an inventory item no quantity of
        // which
        // was received
        //
        if (data.getId() == null && data.getQuantityReceived() == null &&
            data.getReceivedDate() == null)
            return;
        ...
        for (int i = 0; i < list.size(); i++ ) {
            ...
            if (orderId == null) {
                ...
                if ( !orderList.contains(orderId)) {
                    //
                    // this is done in order to make sure that if the inventory
                    // item in the
                    // last DO in the order has not been received the full
                    // quantity of,
                    // the additional DO gets added at the end of the list when
                    // the order changes
                    //
                    if (prevData != null &&
                ...
                if ( !data.getInventoryItemId().equals(prevInvItemId)) {
                    if (prevInvItemId != null && qtyReceived != 0) {
                        ...
                        // the first DO, qtyReceived doesn't stay as 0 if
                        // quantity
