...
public class TurnaroundStatisticReportService {

    public TurnAroundReportViewVO fetchForTurnaroundStatistic(Query query) throws Exception {
        try {
        return EJBFactory.getTurnaroundStatisticReport().fetchForTurnaroundStatistic(query.getFields());
        } catch (Exception e) {
            e.printStackTrace();
            throw e;
        }
    ...
    public ReportStatus runReport(TurnAroundReportViewVO data) throws Exception { 
        ...
        st = EJBFactory.getTurnaroundStatisticReport().runReport(data);
