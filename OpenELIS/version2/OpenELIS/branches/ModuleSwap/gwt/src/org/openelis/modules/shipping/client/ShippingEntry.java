...
public class ShippingEntry implements EntryPoint {
    
    @Override
    public void onModuleLoad() {
        OpenELISEntry.mainBus.addHandler(OpenELIS.SHIPPING, new ShowScreenHandler() {
                ...
                GWT.runAsync(new RunAsyncCallback() {
                    public void onSuccess() {
                        try {
                            ...
                            window.setName(OpenELIS.getMessages().shipping());
                            window.setSize("20px", "20px");
                            window.setContent(new ShippingScreen(window));
                            OpenELIS.getBrowser().addWindow(window, "shipping");
                        } catch (Throwable e) {
