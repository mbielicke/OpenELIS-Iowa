...
public class ExchangeVocabularyMapEntry implements EntryPoint {
    
    @Override
    public void onModuleLoad() {
        OpenELISEntry.mainBus.addHandler(OpenELIS.EXCHANGE_VOCABULARY, new ShowScreenHandler() {
                ...
                GWT.runAsync(new RunAsyncCallback() {
                    public void onSuccess() {
                        try {
                            ...
                            window.setName(Messages.get().exchangeVocabularyMap());
                            window.setSize("20px", "20px");
                            window.setContent(new ExchangeVocabularyMapScreen(window));
                            OpenELIS.getBrowser().addWindow(window, "exchangeVocabularyMap");
                        } catch (Throwable e) {
