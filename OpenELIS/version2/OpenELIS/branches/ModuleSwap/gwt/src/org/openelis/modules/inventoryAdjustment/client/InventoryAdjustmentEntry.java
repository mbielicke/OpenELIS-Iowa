...
public class InventoryAdjustmentEntry implements EntryPoint {
    
    @Override
    public void onModuleLoad() {
        OpenELISEntry.mainBus.addHandler(OpenELIS.INVENTORY_ADJUSTMENT, new ShowScreenHandler() {
                ...
                GWT.runAsync(new RunAsyncCallback() {
                    public void onSuccess() {
                        try {
                            ...
                            window.setName(OpenELIS.getMessages().inventoryAdjustment());
                            window.setSize("20px", "20px");
                            window.setContent(new InventoryAdjustmentScreen(window));
                            OpenELIS.getBrowser().addWindow(window, "inventoryAdjustment");
                        } catch (Throwable e) {
