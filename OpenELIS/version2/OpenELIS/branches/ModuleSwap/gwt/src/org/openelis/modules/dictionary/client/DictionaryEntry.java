...
public class DictionaryEntry implements EntryPoint {
    
    @Override
    public void onModuleLoad() {
        OpenELISEntry.mainBus.addHandler(OpenELIS.DICTIONARY, new ShowScreenHandler() {
                ...
                GWT.runAsync(new RunAsyncCallback() {
                    public void onSuccess() {
                        try {
                            ...
                            window.setName(Messages.get().dictionary());
                            window.setSize("20px", "20px");
                            window.setContent(new DictionaryScreen(window));
                            OpenELIS.getBrowser().addWindow(window, "dictionary");
                        } catch (Throwable e) {
