...
public class SectionEntry implements EntryPoint {
    
    @Override
    public void onModuleLoad() {
        OpenELISEntry.mainBus.addHandler(OpenELIS.SECTION, new ShowScreenHandler() {
                ...
                GWT.runAsync(new RunAsyncCallback() {
                    public void onSuccess() {
                        try {
                            ...
                            window.setName(OpenELIS.getMessages().labSection());
                            window.setSize("20px", "20px");
                            window.setContent(new SectionScreen(window));
                            OpenELIS.getBrowser().addWindow(window, "labSection");
                        } catch (Throwable e) {
