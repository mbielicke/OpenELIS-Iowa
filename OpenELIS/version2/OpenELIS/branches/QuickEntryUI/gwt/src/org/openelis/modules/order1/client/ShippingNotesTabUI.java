...
public class ShippingNotesTabUI extends Screen {

    @UiTemplate("ShippingNotesTab.ui.xml")
    interface ShippingNotesTabUIBinder extends UiBinder<Widget, ShippingNotesTabUI> {
    };
    ...
    protected NoteViewDO                    displayedShippingNote;
    ...
    public ShippingNotesTabUI(Screen parentScreen) {
        ...
        manager = null;
        displayedShippingNote = null;
    }

    private void initialize() {
        addScreenHandler(notePanel, "shippingNotesPanel", new ScreenHandler<String>() {
            public void onDataChange(DataChangeEvent event) {
                drawNote();
            }
        });

        addScreenHandler(editNoteButton, "shippingNoteButton", new ScreenHandler<Object>() {
    ...
    public void onDataChange() {
        ...
        id1 = null;
        txt1 = null;
        if (displayedShippingNote != null) {
            id1 = displayedShippingNote.getId();
            txt1 = displayedShippingNote.getText();
        }
        ...
        if (manager != null && manager.shippingNote.get() != null) {
            ...
            txt2 = manager.shippingNote.get().getText();
        }
    ...
    private void displayNote() {
        ...
        if (redraw) {
            redraw = false;
            if (manager != null)
                displayedShippingNote = manager.shippingNote.get();
            else
                displayedShippingNote = null;
            setState(state);
            fireDataChange();
