...
public class BTProcessor extends Processor {
	
	SampleManager1 sm;
	AnalysisViewDO analysis;
	
	public BTProcessor(SampleManager1 sm, AnalysisViewDO analysis) {
		this.sm = sm;
	...
	public CaseManager process() throws Exception {
		...
		if(isResultFlagged(result.getTestResultId())) {
			caseFactory = new CaseFactory(EJBFactory.getDictionaryCache().getBySystemName("bt_condition").getId());
