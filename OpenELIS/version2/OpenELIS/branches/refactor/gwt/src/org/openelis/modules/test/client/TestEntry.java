...
public class TestEntry implements EntryPoint, ShowScreenHandler {
    
    @Override
    public void onModuleLoad() {
        ScreenBus.get().addHandler(ScreenBus.TEST, this);        
    }
        ...
        GWT.runAsync(new RunAsyncCallback() {
            public void onSuccess() {
                try {
                    ...
                    window.setName(Messages.get().test());
                    window.setSize("20px", "20px");
                    window.setContent(new TestScreen(window));
                    OpenELIS.getBrowser().addWindow(window, "test");
                } catch (Throwable e) {
