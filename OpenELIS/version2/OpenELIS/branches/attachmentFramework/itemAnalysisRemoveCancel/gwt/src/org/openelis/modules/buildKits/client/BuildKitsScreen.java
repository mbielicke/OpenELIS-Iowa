...
public class BuildKitsScreen extends Screen {
    ...
    private void initialize() {
        ...
        addScreenHandler(name, new ScreenEventHandler<Integer>() {
            ...
            public void onValueChange(ValueChangeEvent<Integer> event) {
                ...
                try {
                    /*
                     * set number of kits requested to one by default to prevent
                     * an unintended number of kits be created accidentally
                     */
        ...
        addScreenHandler(numRequested, new ScreenEventHandler<Integer>() {
            ...
            public void onValueChange(ValueChangeEvent<Integer> event) {
                manager.getInventoryReceipt().setQuantityReceived(event.getValue());
            ...
            public void onGetMatches(GetMatchesEvent event) {
                ...
                try {
                    ...
                    if ("Y".equals(addToExisting.getValue()) && itemId != null) {
                        ...
                        window.setBusy();
                        invLocList = InventoryLocationService.get()
                                                             .fetchByLocationNameInventoryItemId(query);
        ...
        componentLocationStorageLocationName.addGetMatchesHandler(new GetMatchesHandler() {
            public void onGetMatches(GetMatchesEvent event) {
                ...
                try {
                    ...
                    query.setFields(fields);
                    invLocList = InventoryLocationService.get()
                                                         .fetchByLocationNameInventoryItemIdStoreId(query);
                    for (i = 0; i < invLocList.size(); i++ ) {
    ...
    private ArrayList<TableDataRow> getTableModel() {
        ...
        try {
            ...
            if (manager.getInventoryReceipt() != null)
                numReq = manager.getInventoryReceipt().getQuantityReceived();
            else
                numReq = null;
            for (int i = 0; i < components.count(); i++ ) {
                data = components.getComponentAt(i);
                row = new TableDataRow(6);
                row.cells.get(0).setValue(data.getComponentName());
                row.cells.get(3).setValue(data.getComponentDispensedUnitsId());
                /*
                 * set the total quantity required for each component if the
                 * number of kits requested has been specified
                 */
                if (numReq != null) {
                    data.setTotal(numReq * data.getQuantity());
                    row.cells.get(4).setValue(data.getTotal());
                }
                ...
                if (data.getInventoryLocationId() != null) {
                    ...
                    row.cells.get(1).setValue(new TableDataRow(data.getInventoryLocationId(),
                                                               location));
                    row.cells.get(2).setValue(data.getInventoryLocationLotNumber());
                    row.cells.get(5).setValue(data.getInventoryLocationQuantityOnhand());
                }
    ...
    private void setTotalInComponents(Integer val) {
        ...
        if (val < 1) {
            numRequested.addException(new Exception(Messages.get()
                                                            .numRequestedMoreThanZeroException()));
            return;
        }
        ...
        if (total == null) {
            ...
            if (changeTotal) {
                ...
                numRequested.addException(new Exception(Messages.get()
                                                                .qtyOnHandNotSufficientException()));
            }
        }
    }
}