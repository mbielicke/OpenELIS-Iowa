...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="sample">
        ...
        <xsl:copy>
            ...
            <xsl:apply-templates select="//sample_external_notes/note[@reference_id = $id]" />
    ...
    <xsl:template match="sample_environmental">
        <xsl:copy>
            ...
            <xsl:apply-templates select="//address[@id = current()/@location_address_id]">
    ...
    <xsl:template match="sample_sdwis">
        <xsl:copy>
            ...
            <xsl:apply-templates select="//dictionary[@id = current()/@sample_category_id]">
    ...
    <xsl:template match="sample_neonatal">
        <xsl:copy>
            ...
            <xsl:apply-templates select="//patient[@id = current()/@next_of_kin_id]">
                ...
                <xsl:with-param name="relation_id">
                    <xsl:value-of select="@next_of_kin_relation_id" />
                </xsl:with-param>
            </xsl:apply-templates>
    ...
    <xsl:template match="sample_organization">
        <xsl:copy>
            ...
            <xsl:apply-templates select="//organization[@id = current()/@organization_id]">
    ...
    <xsl:template match="sample_item">
        <xsl:copy>
            <xsl:copy-of select="@*" />
            <xsl:apply-templates select="@*|node()" />
            <xsl:apply-templates select="//dictionary[@id = current()/@type_of_sample_id]">
                <xsl:with-param name="tagname">type_of_sample</xsl:with-param>
            </xsl:apply-templates>
            <xsl:apply-templates select="//dictionary[@id = current()/@source_of_sample_id]">
            ...
            <xsl:apply-templates select="//dictionary[@id = current()/@unit_of_measure_id]">
    ...
    <xsl:template match="test">
        <xsl:copy>
            ...
            <xsl:apply-templates select="//dictionary[@id = current()/@revision_method_id]">
                <xsl:with-param name="tagname">revision_method</xsl:with-param>
            </xsl:apply-templates>

            <xsl:apply-templates select="//dictionary[@id = current()/@reporting_method_id]">
                <xsl:with-param name="tagname">reporting_method</xsl:with-param>
            </xsl:apply-templates>

            <xsl:apply-templates select="//dictionary[@id = current()/@sorting_method_id]">
                <xsl:with-param name="tagname">sorting_method</xsl:with-param>
            </xsl:apply-templates>

            <xsl:apply-templates select="//test_trailer[@id = current()/@test_trailer_id]" />

            <xsl:apply-templates select="//test_translations/translation[@reference_id = current()/@id]" />
