...
public class DataExchangeReportBean {
    ...
    /**
     * Execute the query defined in the exchange criteria specified by the
     * parameter "name" to generate the list of samples that would be used to
     * create the message sent to the location specified in the exchange
     * criteria. Add an entry to the event log containing the list of samples if
     * there were found any or a message stating any problems encountered duing
     * the whole process.
     */
    @Asynchronous
    ...
    public void runReport(String name) {
        ...
        try {
            ...
            addLogEntry(Messages.get().dataExchangeCriteriaNotFound(name),
                        null,
                        Constants.dictionary().LOG_LEVEL_ERROR,
                        null);
            log.log(Level.SEVERE, Messages.get().dataExchangeCriteriaNotFound(name), e);
            return;
        } catch (Exception e) {
            addLogEntry(Messages.get().dataExchangeCriteriaNotFound(name),
                        null,
                        Constants.dictionary().LOG_LEVEL_ERROR,
                        null);
            log.log(Level.SEVERE, Messages.get().dataExchangeCriteriaFetchFailed(name), e);
            return;
        }
        ...
        try {
            ...
            addLogEntry(Messages.get().dataExchangeLastRunDateFetchFailed(name),
                        null,
                        Constants.dictionary().LOG_LEVEL_ERROR,
                        null);
            log.log(Level.SEVERE, Messages.get().dataExchangeLastRunDateFetchFailed(name), e);
            return;
        }
        ...
        if (fields != null) {
            try {
                ...
                /*
                 * generate the message for the successful execution of the
                 * criteria
                 */
                addLogEntry(Messages.get().dataExchangeExecutedCriteria(name),
                            data.getId(),
                            Constants.dictionary().LOG_TYPE_DATA_TRANSMISSION,
                            text);
            } catch (NotFoundException e) {
                addLogEntry(Messages.get().dataExchangeNoSamplesFound(name),
                            data.getId(),
                            Constants.dictionary().LOG_LEVEL_INFO,
                            null);
            } catch (Exception e) {
                addLogEntry(Messages.get().dataExchangeCouldNotExecuteCriteria(name),
                            data.getId(),
                            Constants.dictionary().LOG_LEVEL_ERROR,
                            null);
                log.log(Level.SEVERE, Messages.get().dataExchangeCouldNotExecuteCriteria(name), e);
            }
        }
    }
    ...
    public ReportStatus exportToLocation(ArrayList<Integer> accessionNumbers, String uri,
        ...
        /*
         * the id of the exchange criteria can be null if the user wants to
         * export to the location in add or default state
         */
    ...
    private void sendMessages(ArrayList<Integer> accessionNumbers, String uri,
        ...
        if (uri.startsWith(FILE_PREFIX)) {
            try {
                ...
                /*
                 * iterate through the list of accession numbers and generate
                 * the xml for each sample
                 */
                for (Integer accNum : accessionNumbers) {
                    man = SampleManager.fetchWithAllDataByAccessionNumber(accNum);

                    tempFile = File.createTempFile(DataBaseUtil.concat(accNum, "_dataExchange"),
                                                   ".xml",
                                                   dir);
    ...
    private void addLogEntry(String message, Integer referenceId, Integer levelId, String text) {
        try {
            eventLog.add(Constants.dictionary().LOG_TYPE_DATA_TRANSMISSION,
                         message,
            ...
            log.log(Level.SEVERE, "Failed to add log entry for: " + message, e);
        }
    }
}