...
<xsl:stylesheet version="1.0" extension-element-prefixes="resource" xmlns:locale="xalan://java.util.Locale"
  xmlns:resource="xalan://org.openelis.util.UTFResource" xmlns:xalan="http://xml.apache.org/xalan"
...
>
  ...
  <xsl:template match="doc">
    ...
    <screen xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" id="testAnalyteLookup"
      name="{resource:getString($constants,'testAnalyteSelection')}"
    >
        ...
        <VerticalPanel style="WhiteContentPanel">
          <TablePanel style="Form">
            <row>
              ...
              <autoComplete field="Integer" key="test" tab="testAnalyteTable,testAnalyteTable"
                width="400" popWidth="auto"
              >
                <col width="400" />
              </autoComplete>
            </row>
          </TablePanel>
          <table key="testAnalyteTable" width="auto" maxRows="10" multiSelect="true"
            showScroll="ALWAYS" title="" style="ScreenTableWithSides"
          >
            <col width="400" header="{resource:getString($constants,'analyte')}">
              <label field="String" />
            </col>
            <col width="50" header="{resource:getString($constants,'row')}">
              <label field="Integer" />
            </col>
            <col width="50" header="{resource:getString($constants,'column')}">
              <label field="Integer" />
            </col>
          </table>
        </VerticalPanel>
