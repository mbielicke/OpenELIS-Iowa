...
public class ExchangeCriteriaManagerBean {
    ...
    public ExchangeCriteriaManager update(ExchangeCriteriaManager man) throws Exception {
        ...
        try {
            ut.begin();
            lock.validateLock(Constants.table().EXCHANGE_CRITERIA,
                              man.getExchangeCriteria().getId());
            man.update();
            lock.unlock(Constants.table().EXCHANGE_CRITERIA, man.getExchangeCriteria()
                                                                .getId());
            ut.commit();
        } catch (Exception e) {
            ut.rollback();
    ...
    public ExchangeCriteriaManager fetchForUpdate(Integer id) throws Exception {
        ...
        try {
            ut.begin();
            lock.lock(Constants.table().EXCHANGE_CRITERIA, id);
            man = fetchById(id);
            ut.commit();
            return man;
        } catch (Exception e) {
    ...
    public ExchangeCriteriaManager abortUpdate(Integer id) throws Exception {
        lock.unlock(Constants.table().EXCHANGE_CRITERIA, id);
        return fetchById(id);
    }
    ...
    private void duplicateProfiles(ExchangeProfileManager oldMan,
                                   ExchangeProfileManager newMan) {
