...
public class SDWISUnloadReportBean {
                    ...
                    while (aIter.hasNext()) {
                        ...
                        if ( Constants.dictionary().ANALYSIS_RELEASED.equals(surVO.getStatusId()) &&
                            ...
                            try {
                                ...
                                // no qa events found means analysis is not
                                // overridden
                            } catch (Exception anyE) {
    ...
    protected void writeSampleRow(PrintWriter writer, SampleDO sVDO,
        ...
        String compDateString, compIndicator, compLabNumber, compQuarter, freeChlorine, pbType, repeatCode, totalChlorine, origSampleNumber, sampleOverrideString;
        StringBuilder row;
        ...
        try {
            adList = auxData.fetchById(sVDO.getId(), Constants.table().SAMPLE);
        } catch (Exception anyE) {
            throw new Exception("Error retrieving auxillary data for accession #" +
                                sVDO.getAccessionNumber() + "; " + anyE.getMessage());
        }
        ...
        while (adIter.hasNext()) {
            ...
            if ("pb_type".equals(adVDO.getAnalyteExternalId())) {
                if (adVDO.getValue() != null && adVDO.getValue().length() > 0) {
                    try {
                        pbType = dictionaryCache.getById(Integer.valueOf(adVDO.getValue()))
                                                .getCode();
                    } catch (Exception anyE) {
                        throw new Exception("Error looking up dictionary entry for Pb Sample Type; " +
                                            anyE.getMessage());
                    }
                }
                ...
                if (adVDO.getValue() != null && adVDO.getValue().length() > 0) {
                    try {
                        ...
                        throw new Exception("Error looking up dictionary entry for Repeat Code; " +
                                            anyE.getMessage());
                    }
                }
                ...
                if (adVDO.getValue() != null && adVDO.getValue().length() > 0) {
                    try {
                        compIndicator = dictionaryCache.getById(Integer.valueOf(adVDO.getValue()))
                                                       .getCode();
                    } catch (Exception anyE) {
                        throw new Exception("Error looking up dictionary entry for Composite Indicator; " +
                                            anyE.getMessage());
                    }
                }
        ...
        row.append("#SAM") // col 1-4
           ...
           // col 33-43
           .append(getPaddedString(ssVDO.getLocation(), 20))
           // col 44-63
           .append(getPaddedString(ReportUtil.toString(sVDO.getCollectionDate(),
                                                       "yyyyMMdd"), 8))
           // col 64-71
           .append(getPaddedString(ReportUtil.toString(sVDO.getCollectionTime(), "HHmm"),
                                   4))
           // col 72-75
           .append(getPaddedString(ssVDO.getCollector(), 20))
           // col 76-95
           .append(getPaddedString(ReportUtil.toString(sVDO.getReceivedDate(), "yyyyMMdd"),
                                   8))
           // col 96-103
    ...
    protected void writeResultRows(PrintWriter writer, SampleDO sample,
        ...
        try {
            ...
            throw new Exception("Error looking up dictionary entry for Sample Category; " +
                                anyE.getMessage());
        }
        ...
        while (rowIter.hasNext()) {
            ...
            if (Constants.dictionary().SDWIS_CATEGORY_BACTERIAL.equals(sampCatDO.getId())) {
                if (Constants.dictionary().TEST_RES_TYPE_DICTIONARY.equals(rVDO.getTypeId())) {
                    try {
                        rowData.put("microbe",
                                    dictionaryCache.getById(Integer.valueOf(rVDO.getValue()))
                                                   .getEntry());
                    } catch (Exception anyE) {
        ...
        for (i = 0; i < resultData.size(); i++ ) {
            ...
            row.append("#RES") // col 1-4
               ...
               .append(getPaddedString(ReportUtil.toString(surVO.getStartedDate(),
                                                           "yyyyMMdd"), 8))
               // col 21-28
               .append(getPaddedString(ReportUtil.toString(surVO.getStartedDate(), "HHmm"),
                                       4))
               // col 29-32
