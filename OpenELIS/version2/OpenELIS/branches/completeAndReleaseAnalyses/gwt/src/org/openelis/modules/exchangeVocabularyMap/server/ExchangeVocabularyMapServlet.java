...
/*
 * This class provides service for ExchangeExternalTermManager and
 * ExchangeLocalTermManager
 */
@WebServlet("/openelis/exchangeVocabulary")
public class ExchangeVocabularyMapServlet extends RemoteServlet implements
                                                               ExchangeVocabularyMapServiceInt {

    private static final long            serialVersionUID = 1L;

    @EJB
    private ExchangeLocalTermManagerBean exchangeLocalTermManager;

    @EJB
    private ExchangeLocalTermBean        exchangeLocalTerm;
    ...
    public ArrayList<TestAnalyteViewVO> fetchTestAnalytes(ArrayList<QueryData> fields) throws Exception {
        try {
            return exchangeLocalTerm.fetchTestAnalytes(fields);
        } catch (Exception anyE) {
            throw serializeForGWT(anyE);
        }
    }
