...
/**
 * This class manages the widgets used for verifying sdwis facility id
 */
public class SDWISFacilityId extends SingleField<TextBox<String>> {
    public SDWISFacilityId(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    protected void init() {
        ...
        key = SampleMeta.SDWIS_FACILITY_ID;
    ...
    /**
     * Copies sdwis facility id from the manager to the editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleSDWIS().getFacilityId())) {
            editableWidget.setValue(sm.getSampleSDWIS().getFacilityId());
    ...
    /**
     * Copies sdwis facility id from the editable widget to the manager
     */
    public void copyToSample() {
        ...
        sm = parentScreen.getManager();
        facId = editableWidget.getValue();
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(facId, sm.getSampleSDWIS().getFacilityId())) {
            sm.getSampleSDWIS().setFacilityId(facId);
            nonEditableWidget.setValue(facId);
            matchImage.setResource(OpenELISResources.INSTANCE.commit());
     ...
     * sample's sdwis facility id; increments the number of times the value has
     * been changed; if the values are different and the value has been changed
     * more than once, shows the sample's sdwis facility id to the user
     */
