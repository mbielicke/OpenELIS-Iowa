 ...
 */
public class EnvLocation extends SingleField<TextBox<String>> {
    public EnvLocation(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    /**
     * Copies environmental location from the manager to the editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleEnvironmental()
                                                                  .getLocation())) {
            editableWidget.setValue(sm.getSampleEnvironmental().getLocation());
    ...
    /**
     * Copies environmental location from the editable widget to the manager
     */
    public void copyToSample() {
        ...
        sm = parentScreen.getManager();
        loc = editableWidget.getValue();
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(loc, sm.getSampleEnvironmental().getLocation())) {
            sm.getSampleEnvironmental().setLocation(loc);
            nonEditableWidget.setValue(loc);
            matchImage.setResource(OpenELISResources.INSTANCE.commit());
     ...
     * sample's environmental location; increments the number of times the value
     * has been changed; if the values are different and the value has been
     * changed more than once, shows the sample's environmental location to the
     * user
     */
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's environmental location
     */
    public void verify() {
