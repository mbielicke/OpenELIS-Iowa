...
/**
 * This class manages the widgets used for verifying sdwis sample category id
 */
public class SDWISSampleCategoryId extends SingleField<Dropdown<Integer>> {
    public SDWISSampleCategoryId(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    /**
     * Copies sdwis sample category id from the manager to the editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleSDWIS()
                                                                  .getSampleCategoryId())) {
            editableWidget.setValue(sm.getSampleSDWIS().getSampleCategoryId());
    ...
    /**
     * Copies sdwis sample category id from the editable widget to the manager
     */
    public void copyToSample() {
        ...
        sm = parentScreen.getManager();
        catId = editableWidget.getValue();
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(catId, sm.getSampleSDWIS().getSampleCategoryId())) {
            sm.getSampleSDWIS().setSampleCategoryId(catId);
            nonEditableWidget.setValue(catId);
            matchImage.setResource(OpenELISResources.INSTANCE.commit());
     ...
     * sample's sdwis sample category id; increments the number of times the
     * value has been changed; if the values are different and the value has
     * been changed more than once, shows the sample's sdwis sample category id
     * to the user
     */
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's sdwis sample category id
     */
    public void verify() {
