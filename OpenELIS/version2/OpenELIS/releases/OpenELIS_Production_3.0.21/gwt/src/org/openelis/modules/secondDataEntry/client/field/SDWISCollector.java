...
/**
 * This class manages the widgets used for verifying sdwis collector
 */
public class SDWISCollector extends SingleField<TextBox<String>> {
    public SDWISCollector(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    protected void init() {
        ...
        key = SampleMeta.SDWIS_COLLECTOR;
    ...
    /**
     * Copies sdwis collector from the manager to the editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleSDWIS().getCollector())) {
            editableWidget.setValue(sm.getSampleSDWIS().getCollector());
    ...
    /**
     * Copies sdwis collector from the editable widget to the manager
     */
    public void copyToSample() {
        ...
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(col, sm.getSampleSDWIS().getCollector())) {
            sm.getSampleSDWIS().setCollector(col);
     ...
     * sample's sdwis collector; increments the number of times the value has
     * been changed; if the values are different and the value has been changed
     * more than once, shows the sample's sdwis collector to the user
     */
