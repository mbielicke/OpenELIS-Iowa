 ...
 * address zip code
 */
public class EnvLocationAddrZipCode extends SingleField<TextBox<String>> {
    public EnvLocationAddrZipCode(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    protected void init() {
        ...
        key = SampleMeta.LOCATION_ADDR_ZIP_CODE;
    ...
    /**
     * Copies environmental location address zip code from the manager to the
     * editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleEnvironmental()
                                                                  .getLocationAddress()
                                                                  .getZipCode())) {
            editableWidget.setValue(sm.getSampleEnvironmental().getLocationAddress().getZipCode());
    ...
    /**
     * Copies environmental location address zip code from the editable widget
     * to the manager
     */
    public void copyToSample() {
        ...
        sm = parentScreen.getManager();
        zip = editableWidget.getValue();
        if (numEdit > 1 &&
            !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(zip, sm.getSampleEnvironmental()
                                            .getLocationAddress()
                                            .getZipCode())) {
            sm.getSampleEnvironmental().getLocationAddress().setZipCode(zip);
            nonEditableWidget.setValue(zip);
            matchImage.setResource(OpenELISResources.INSTANCE.commit());
     ...
     * sample's environmental location address zip code; increments the number
     ...
     * location address zip code to the user
     */
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's environmental location address zip code
     */
    public void verify() {
