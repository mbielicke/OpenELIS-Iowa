...
/**
 * This class manages the widgets used for verifying pt due date
 */
public class PTDueDate extends SingleField<Calendar> {
    public PTDueDate(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    /**
     * Copies pt due date from the manager to the editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferentYM(editableWidget.getValue(), sm.getSamplePT().getDueDate())) {
            editableWidget.setValue(sm.getSamplePT().getDueDate());
    ...
    /**
     * Copies pt due date from the editable widget to the manager
     */
    public void copyToSample() {
        ...
        sm = parentScreen.getManager();
        ddt = editableWidget.getValue();
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferentYM(ddt, sm.getSamplePT().getDueDate())) {
            sm.getSamplePT().setDueDate(ddt);
            nonEditableWidget.setValue(ddt);
            matchImage.setResource(OpenELISResources.INSTANCE.commit());
     ...
     * sample's pt due date increments the number of times the value has been
     * changed; if the values are different and the value has been changed more
     * than once, shows the sample's pt due date to the user
     */
    public void valueChanged() {
        ...
        if ( !isVerified) {
            ...
            if (numEdit > 1)
                nonEditableWidget.setValue(parentScreen.getManager().getSamplePT().getDueDate());
            /*
