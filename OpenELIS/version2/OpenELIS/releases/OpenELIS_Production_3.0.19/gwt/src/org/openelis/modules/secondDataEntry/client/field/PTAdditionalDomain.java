...
/**
 * This class manages the widgets used for verifying pt additional domain
 */
public class PTAdditionalDomain extends SingleField<Dropdown<String>> {
    public PTAdditionalDomain(VerificationScreen parentScreen, TableRowElement tableRowElement,
                              Dropdown<String> editableWidget, Dropdown<String> nonEditableWidget,
    ...
    protected void init() {
        ...
        key = SampleMeta.PT_ADDITIONAL_DOMAIN;
    ...
    /**
     * Copies pt additional domain from the manager to the editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSamplePT()
                                                                  .getAdditionalDomain())) {
            editableWidget.setValue(sm.getSamplePT().getAdditionalDomain());
    ...
    /**
     * Copies pt additional domain from the editable widget to the manager
     */
    public void copyToSample() {
        ...
        sm = parentScreen.getManager();
        dom = editableWidget.getValue();
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(dom, sm.getSamplePT().getAdditionalDomain())) {
            sm.getSamplePT().setAdditionalDomain(dom);
            nonEditableWidget.setValue(dom);
            matchImage.setResource(OpenELISResources.INSTANCE.commit());
     ...
     * sample's pt additional domain; increments the number of times the value
     * has been changed; if the values are different and the value has been
     * changed more than once, shows the sample's pt additional domain to the
     * user
     */
