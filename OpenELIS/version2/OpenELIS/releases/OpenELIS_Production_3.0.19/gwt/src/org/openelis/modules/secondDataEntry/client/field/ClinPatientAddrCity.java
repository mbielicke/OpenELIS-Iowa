 ...
 * city
 */
public class ClinPatientAddrCity extends SingleField<TextBox<String>> {
    public ClinPatientAddrCity(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    protected void init() {
        ...
        key = SampleMeta.CLIN_PATIENT_ADDR_CITY;
        ...
        parentScreen.getEventBus().addHandler(PatientChangeEvent.getType(),
                                              new PatientChangeEvent.Handler() {
                                                  ...
                                                  public void onPatientChange(PatientChangeEvent event) {
                                                       ...
                                                       * new patient's city; if
                                                       * the previous patient's
                                                       * city was being shown in
                                                       * the non editable
                                                       * widget, replace that
                                                       * with the new patient's
                                                       * city; if no value was
    ...
    /**
     * Copies clinical patient address city from the manager to the editable
     * widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleClinical()
                                                                  ...
                                                                  .getAddress()
                                                                  .getCity())) {
            editableWidget.setValue(sm.getSampleClinical().getPatient().getAddress().getCity());
    ...
    /**
     * Copies clinical patient address city from the editable widget to the
     * manager
     */
    public void copyToSample() {
        ...
        if (numEdit > 1 &&
            ...
            DataBaseUtil.isDifferent(city, sm.getSampleClinical()
                                             .getPatient()
                                             .getAddress()
                                             .getCity())) {
            sm.getSampleClinical().getPatient().getAddress().setCity(city);
     ...
     * sample's clinical patient address city; increments the number of times
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's clinical patient address city
     */
    public void verify() {
