...
/**
 * This class manages the widgets used for verifying environmental collector
 * phone
 */
public class EnvCollectorPhone extends SingleField<TextBox<String>> {
    public EnvCollectorPhone(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    protected void init() {
        ...
        key = SampleMeta.ENV_COLLECTOR_PHONE;
    ...
    /**
     * Copies environmental collector phone from the manager to the editable
     * widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleEnvironmental()
                                                                  .getCollectorPhone())) {
            editableWidget.setValue(sm.getSampleEnvironmental().getCollectorPhone());
    ...
    /**
     * Copies environmental collector phone from the editable widget to the
     * manager
     */
    public void copyToSample() {
        ...
        sm = parentScreen.getManager();
        col = editableWidget.getValue();
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(col, sm.getSampleEnvironmental().getCollectorPhone())) {
            sm.getSampleEnvironmental().setCollectorPhone(col);
            nonEditableWidget.setValue(col);
            matchImage.setResource(OpenELISResources.INSTANCE.commit());
     ...
     * sample's environmental collector phone; increments the number of times
     * the value has been changed; if the values are different and the value has
     * been changed more than once, shows the sample's environmental collector
     * phone to the user
     */
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's environmental collector phone
     */
    public void verify() {
