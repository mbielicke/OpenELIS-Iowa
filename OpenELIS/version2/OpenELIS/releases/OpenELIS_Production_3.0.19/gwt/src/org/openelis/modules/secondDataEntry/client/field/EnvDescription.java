...
/**
 * This class manages the widgets used for verifying environmental description
 */
public class EnvDescription extends SingleField<TextBox<String>> {
    public EnvDescription(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    protected void init() {
        ...
        key = SampleMeta.ENV_DESCRIPTION;
    ...
    /**
     * Copies environmental description from the manager to the editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleEnvironmental()
                                                                  .getDescription())) {
            editableWidget.setValue(sm.getSampleEnvironmental().getDescription());
    ...
    /**
     * Copies environmental description from the editable widget to the manager
     */
    public void copyToSample() {
        ...
        sm = parentScreen.getManager();
        desc = editableWidget.getValue();
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(desc, sm.getSampleEnvironmental().getDescription())) {
            sm.getSampleEnvironmental().setDescription(desc);
            nonEditableWidget.setValue(desc);
            matchImage.setResource(OpenELISResources.INSTANCE.commit());
     ...
     * sample's environmental description; increments the number of times the
     * value has been changed; if the values are different and the value has
     * been changed more than once, shows the sample's environmental description
     * to the user
     */
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's environmental description
     */
    public void verify() {
