 ...
 * address state
 */
public class EnvLocationAddrState extends SingleField<Dropdown<String>> {
    public EnvLocationAddrState(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    protected void init() {
        ...
        key = SampleMeta.LOCATION_ADDR_STATE;
    ...
    /**
     * Copies environmental location address state from the manager to the
     * editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleEnvironmental()
                                                                  .getLocationAddress()
                                                                  .getState())) {
            editableWidget.setValue(sm.getSampleEnvironmental().getLocationAddress().getState());
    ...
    /**
     * Copies environmental location address state from the editable widget to
     * the manager
     */
    public void copyToSample() {
        ...
        if (numEdit > 1 &&
            ...
            DataBaseUtil.isDifferent(state, sm.getSampleEnvironmental()
                                              .getLocationAddress()
                                              .getState())) {
            sm.getSampleEnvironmental().getLocationAddress().setState(state);
     ...
     * sample's environmental location address state; increments the number of
     ...
     * location address state to the user
     */
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's environmental location address state
     */
    public void verify() {
