...
/**
 * This class manages the widgets used for verifying client reference
 */
public class ClientReference extends SingleField<TextBox<String>> {
    public ClientReference(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    protected void init() {
        ...
        key = SampleMeta.CLIENT_REFERENCE;
    ...
    /**
     * Copies client reference from the editable widget to the manager
     */
    public void copyToSample() {
        ...
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(ref, sm.getSample().getClientReference())) {
            sm.getSample().setClientReference(ref);
     ...
     * sample's client reference; increments the number of times the value has
     * been changed; if the values are different and the value has been changed
     * more than once, shows the sample's client reference to the user
     */
