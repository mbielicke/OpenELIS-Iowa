...
/**
 * This class manages the widgets used for verifying environmental priority
 */
public class EnvPriority extends SingleField<TextBox<Integer>> {
    public EnvPriority(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    /**
     * Copies enviromental priority from the manager to the editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleEnvironmental()
                                                                  .getPriority())) {
            editableWidget.setValue(sm.getSampleEnvironmental().getPriority());
    ...
    /**
     * Copies enviromental priority from the editable widget to the manager
     */
    public void copyToSample() {
        ...
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(pr, sm.getSampleEnvironmental().getPriority())) {
            sm.getSampleEnvironmental().setPriority(pr);
     ...
     * sample's enviromental priority
     */
    public void verify() {
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's enviromental priority; increments the number of times the value
     * has been changed; if the values are different and the value has been
     * changed more than once, shows the sample's enviromental priority to the
     * user
     */
