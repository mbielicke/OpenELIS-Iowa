...
/**
 * This class manages the widgets used for verifying pt series address city
 */
public class PTSeries extends SingleField<TextBox<String>> {
    public PTSeries(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    /**
     * Copies pt series from the manager to the editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSamplePT().getSeries())) {
            editableWidget.setValue(sm.getSamplePT().getSeries());
    ...
    /**
     * Copies pt series from the editable widget to the manager
     */
    public void copyToSample() {
        ...
        sm = parentScreen.getManager();
        ser = editableWidget.getValue();
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(ser, sm.getSamplePT().getSeries())) {
            sm.getSamplePT().setSeries(ser);
            nonEditableWidget.setValue(ser);
            matchImage.setResource(OpenELISResources.INSTANCE.commit());
     ...
     * sample's pt series; increments the number of times the value has been
     * changed; if the values are different and the value has been changed more
     * than once, shows the sample's environmental location address city to the
     * user
     */
    public void valueChanged() {
        ...
        if ( !isVerified) {
            ...
            if (numEdit > 1)
                nonEditableWidget.setValue(parentScreen.getManager().getSamplePT().getSeries());
            /*
    ...
    public void verify() {
        isVerified = !DataBaseUtil.isDifferent(editableWidget.getValue(),
                                               parentScreen.getManager().getSamplePT().getSeries());
