...
/**
 * This class manages the widgets used for verifying clinical patient national
 * id
 */
public class ClinPatientNationalId extends SingleField<TextBox<String>> {
    public ClinPatientNationalId(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    protected void init() {
        ...
        key = SampleMeta.CLIN_PATIENT_NATIONAL_ID;
        ...
        parentScreen.getEventBus().addHandler(PatientChangeEvent.getType(),
                                              new PatientChangeEvent.Handler() {
                                                  ...
                                                  public void onPatientChange(PatientChangeEvent event) {
                                                       ...
                                                       * new patient's NID; if
                                                       * the previous patient's
                                                       * NID was being shown in
                                                       * the non editable
                                                       * widget, replace that
    ...
    /**
     * Copies clinical patient national id from the manager to the editable
     * widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleClinical()
                                                                  .getPatient()
                                                                  .getNationalId())) {
            editableWidget.setValue(sm.getSampleClinical().getPatient().getNationalId());
    ...
    /**
     * Copies clinical patient national id from the editable widget to the
     * manager if the national id hasn't been used for another patient;
    ...
    public void copyToSample() {
        ...
        PatientDO data;
        Query query;
        ArrayList<PatientDO> pats;

        data = parentScreen.getManager().getSampleClinical().getPatient();
        nid = editableWidget.getValue();
        if (numEdit <= 1 || editableWidget.hasExceptions() ||
            !DataBaseUtil.isDifferent(nid, data.getNationalId()))
            return;

        if ( !DataBaseUtil.isEmpty(nid)) {
            query = new Query();
            query.setFields(new QueryData(PatientMeta.getNationalId(), QueryData.Type.STRING, nid));
            try {
                pats = PatientService.get().query(query);
                if ( !pats.get(0).getId().equals(data.getId())) {
                    Window.alert(Messages.get().patientLookup_nidUsedForOtherPatient());
                    return;
                }
        ...
        data.setNationalId(nid);
        nonEditableWidget.setValue(nid);
        matchImage.setResource(OpenELISResources.INSTANCE.commit());
     ...
     * sample's clinical patient national id
     */
    public void verify() {
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's clinical patient national id; increments the number of times the
     * value has been changed; if the values are different and the value has
     * been changed more than once, shows the sample's clinical patient national
     * id to the user
     */
