 ...
 * address country
 */
public class EnvLocationAddrCountry extends SingleField<Dropdown<String>> {
    public EnvLocationAddrCountry(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    protected void init() {
        ...
        key = SampleMeta.LOCATION_ADDR_COUNTRY;
    ...
    /**
     * Copies environmental location address country from the manager to the
     * editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleEnvironmental()
                                                                  .getLocationAddress()
                                                                  .getCountry())) {
            editableWidget.setValue(sm.getSampleEnvironmental().getLocationAddress().getCountry());
    ...
    /**
     * Copies environmental location address country from the editable widget to
     * the manager
     */
    public void copyToSample() {
        ...
        sm = parentScreen.getManager();
        cntry = editableWidget.getValue();
        if (numEdit > 1 &&
            !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(cntry, sm.getSampleEnvironmental()
                                              .getLocationAddress()
                                              .getCountry())) {
            sm.getSampleEnvironmental().getLocationAddress().setCountry(cntry);
            nonEditableWidget.setValue(cntry);
            matchImage.setResource(OpenELISResources.INSTANCE.commit());
     ...
     * sample's environmental location address country; increments the number of
     * times the value has been changed; if the values are different and the
     * value has been changed more than once, shows the sample's environmental
     * location address country to the user
     */
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's environmental location address country
     */
    public void verify() {
