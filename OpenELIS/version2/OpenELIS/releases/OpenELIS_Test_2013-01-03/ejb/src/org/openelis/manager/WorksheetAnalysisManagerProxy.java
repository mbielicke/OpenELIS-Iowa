...
public class WorksheetAnalysisManagerProxy {
    ...
    public int add(WorksheetAnalysisManager manager, HashMap<Integer, Integer> idHash) throws Exception {
        ...
        for (i = 0; i < manager.count(); i++) {
            ...
            if (analysis.getWorksheetAnalysisId() != null &&
                analysis.getWorksheetAnalysisId() < 0) {
    ...
    public void add(WorksheetAnalysisManager manager, WorksheetAnalysisDO analysis, int i) throws Exception {
        ...
        if (analysis.getQcLotId() != null) {
            ...
            if (accessionNumber.startsWith("X."))
                analysis.setAccessionNumber(accessionNumber.replaceFirst("X",
            ...
            if (sManager == null) {
                sample = EJBFactory.getSample()
                                   .fetchByAccessionNumber(Integer.valueOf(accessionNumber));
    ...
    public WorksheetAnalysisManager update(WorksheetAnalysisManager manager) throws Exception {
        ...
        for (i = 0; i < manager.count(); i++) {
            ...
            if (analysis.getId() == null) {
                ...
                if (analysis.getAnalysisId() != null) {
                    ...
                    if (sManager == null) {
                        sample = EJBFactory.getSample()
                                           .fetchByAccessionNumber(Integer.valueOf(accessionNumber));
                        sManager = EJBFactory.getSampleManager()
                                             .fetchForUpdate(sample.getId());
                        manager.getLockedManagers().put(sample.getAccessionNumber(),
                                                        sManager);
                        manager.getSampleManagers().put(sample.getAccessionNumber(),
                                                        sManager);
                    }
                    siManager = sManager.getSampleItems();
                        ...
                        for (l = 0; l < aManager.count(); l++) {
                            ...
                            if (analysis.getAnalysisId().equals(aVDO.getId())) {
                                if (Constants.dictionary().WORKSHEET_FAILED.equals(manager.getWorksheet()
                                                                                          .getStatusId())) {
                                    if (Constants.dictionary().ANALYSIS_INITIATED.equals(aVDO.getStatusId()) ||
                                        Constants.dictionary().ANALYSIS_ERROR_INITIATED.equals(aVDO.getStatusId()))
                                        aVDO.setStatusId(Constants.dictionary().ANALYSIS_REQUEUE);
                                } else if (Constants.dictionary().WORKSHEET_VOID.equals(manager.getWorksheet()
                                                                                               .getStatusId())) {
                                    if (Constants.dictionary().ANALYSIS_INITIATED.equals(aVDO.getStatusId()) ||
                                        Constants.dictionary().ANALYSIS_ERROR_INITIATED.equals(aVDO.getStatusId())) {
                                        createdDate = manager.getWorksheet()
                                                             .getCreatedDate();
                                    ...
                                    if (Constants.dictionary().ANALYSIS_LOGGED_IN.equals(aVDO.getStatusId()) ||
                                        ...
                                        Constants.dictionary().ANALYSIS_COMPLETED.equals(aVDO.getStatusId())) {
                                        try {
            ...
            } else if (analysis.getQcLotId() != null &&
                       listItem.worksheetQcResult != null) {
                manager.getWorksheetQcResultAt(i)
                       .setWorksheetAnalysisId(analysis.getId());
     ...
     */
    private void initializeWorksheetResults(AnalysisViewDO aVDO,
                                            AnalysisResultManager arManager,
        ...
        for (i = 0; i < results.size(); i++) {
            ...
            if (twAnalytes.size() == 0 ||
                !twAnalytes.containsKey(result.getTestAnalyteId())) {
...
*/
     ...
     */
    private void initializeWorksheetQcResults(QcManager qcManager,
                                              WorksheetQcResultManager wqrManager) throws Exception {
        int i;
