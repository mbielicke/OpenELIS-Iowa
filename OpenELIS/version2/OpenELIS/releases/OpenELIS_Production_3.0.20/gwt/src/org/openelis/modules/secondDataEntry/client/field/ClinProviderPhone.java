...
/**
 * This class manages the widgets used for verifying clinical provider phone
 */
public class ClinProviderPhone extends SingleField<TextBox<String>> {
    public ClinProviderPhone(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    protected void init() {
        ...
        key = SampleMeta.CLIN_PROVIDER_PHONE;
    ...
    /**
     * Copies clinical provider phone from the manager to the editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleClinical()
                                                                  .getProviderPhone())) {
            editableWidget.setValue(sm.getSampleClinical().getProviderPhone());
    ...
    /**
     * Copies clinical provider phone from the editable widget to the manager
     */
    public void copyToSample() {
        ...
        sm = parentScreen.getManager();
        phone = editableWidget.getValue();
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(phone, sm.getSampleClinical().getProviderPhone())) {
            sm.getSampleClinical().setProviderPhone(phone);
            nonEditableWidget.setValue(phone);
            matchImage.setResource(OpenELISResources.INSTANCE.commit());
     ...
     * sample's clinical provider phone; increments the number of times the
     * value has been changed; if the values are different and the value has
     * been changed more than once, shows the sample's clinical provider phone
     * to the user
     */
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's clinical provider phone
     */
    public void verify() {
