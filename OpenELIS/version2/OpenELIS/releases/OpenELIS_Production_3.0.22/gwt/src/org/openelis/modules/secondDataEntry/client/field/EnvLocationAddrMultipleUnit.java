...
/**
 * This class manages the widgets used for verifying environmental location
 * address multiple unit
 */
public class EnvLocationAddrMultipleUnit extends SingleField<TextBox<String>> {
    public EnvLocationAddrMultipleUnit(VerificationScreen parentScreen,
    ...
    protected void init() {
        ...
        key = SampleMeta.LOCATION_ADDR_MULTIPLE_UNIT;
    ...
    /**
     * Copies environmental location address multiple unit from the manager to
     * the editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleEnvironmental()
                                                                  .getLocationAddress()
                                                                  .getMultipleUnit())) {
            editableWidget.setValue(sm.getSampleEnvironmental()
                                      .getLocationAddress()
                                      .getMultipleUnit());
    ...
    /**
     * Copies environmental location address multiple unit from the editable
     * widget to the manager
     */
    public void copyToSample() {
        ...
        sm = parentScreen.getManager();
        multi = editableWidget.getValue();
        if (numEdit > 1 &&
            !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(multi, sm.getSampleEnvironmental()
                                              .getLocationAddress()
                                              .getMultipleUnit())) {
            sm.getSampleEnvironmental().getLocationAddress().setMultipleUnit(multi);
            nonEditableWidget.setValue(multi);
            matchImage.setResource(OpenELISResources.INSTANCE.commit());
     ...
     * sample's environmental location address multiple unit; increments the
     * number of times the value has been changed; if the values are different
     * and the value has been changed more than once, shows the sample's
     * environmental location address multiple unit to the user
     */
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's environmental location address multiple unit
     */
    public void verify() {
