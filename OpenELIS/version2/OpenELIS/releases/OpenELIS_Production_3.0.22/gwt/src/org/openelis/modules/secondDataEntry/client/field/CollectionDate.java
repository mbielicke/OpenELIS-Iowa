...
/**
 * This class manages the widgets used for verifying collection date
 */
public class CollectionDate extends SingleField<Calendar> {
    public CollectionDate(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    protected void init() {
        ...
        key = SampleMeta.COLLECTION_DATE;
    ...
    /**
     * Copies collection date from the manager to the editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferentYD(editableWidget.getValue(), sm.getSample()
                                                                    .getCollectionDate())) {
            editableWidget.setValue(sm.getSample().getCollectionDate());
    ...
    /**
     * Copies collection date from the editable widget to the manager
     */
    public void copyToSample() {
        ...
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferentYD(cdt, sm.getSample().getCollectionDate())) {
            sm.getSample().setCollectionDate(cdt);
     ...
     * sample's collection date increments the number of times the value has
     * been changed; if the values are different and the value has been changed
     * more than once, shows the sample's collection date to the user
     */
