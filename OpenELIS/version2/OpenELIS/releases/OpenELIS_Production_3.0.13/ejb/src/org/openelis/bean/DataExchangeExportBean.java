...
public class DataExchangeExportBean {
    ...
    public ReportStatus export(SampleQcVO sqc) throws Exception {
        ...
        status.setMessage("XML written to " + cm.getExchangeCriteria().getDestinationUri());
        return status;
    }
    ...
    private void messageOutput(SampleManager1 sm, ExchangeCriteriaManager cm, Date releaseStart,
        ...
        try {
             ...
             * error
             */
            log.log(Level.INFO, "Generating xml for accession number:  " +
                                getSample(sm).getAccessionNumber());
             ...
             */
            log.log(Level.INFO, "Sending transformed xml to the destination:  " +
            ...
            log.log(Level.INFO, "XML sent to the destination");
        } finally {
            if (out != null)
                out.close();
            if (outsocket != null)
