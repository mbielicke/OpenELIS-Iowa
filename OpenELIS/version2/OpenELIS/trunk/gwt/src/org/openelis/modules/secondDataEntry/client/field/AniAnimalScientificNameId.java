...
/**
 * This class manages the widgets used for verifying animal scientific name id
 */
public class AniAnimalScientificNameId extends SingleField<Dropdown<Integer>> {
    public AniAnimalScientificNameId(VerificationScreen parentScreen,
    ...
    protected void init() {
        ...
        setRowVisible();
        key = SampleMeta.ANI_ANIMAL_SCIENTIFIC_NAME_ID;
    ...
    /**
     * Copies animal scientific name id from the manager to the editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleAnimal()
                                                                  .getAnimalScientificNameId())) {
            editableWidget.setValue(sm.getSampleAnimal().getAnimalScientificNameId());
    ...
    /**
     * Copies animal scientific name id from the editable widget to the manager
     */
    public void copyToSample() {
        ...
        sm = parentScreen.getManager();
        typeId = editableWidget.getValue();
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(typeId, sm.getSampleAnimal().getAnimalScientificNameId())) {
            sm.getSampleAnimal().setAnimalScientificNameId(typeId);
            nonEditableWidget.setValue(typeId);
            matchImage.setResource(OpenELISResources.INSTANCE.commit());
     ...
     * sample's animal sample type id; increments the number of times the value
     * has been changed; if the values are different and the value has been
     * changed more than once, shows the sample's animal sample type id to the
     * user
     */
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's animal scientific name id
     */
    public void verify() {
