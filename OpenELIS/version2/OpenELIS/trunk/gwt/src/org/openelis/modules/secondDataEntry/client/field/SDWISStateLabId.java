...
/**
 * This class manages the widgets used for verifying sdwis state lab id
 */
public class SDWISStateLabId extends SingleField<TextBox<Integer>> {
    public SDWISStateLabId(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    /**
     * Copies sdwis state lab id from the manager to the editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleSDWIS().getStateLabId())) {
            editableWidget.setValue(sm.getSampleSDWIS().getStateLabId());
    ...
    /**
     * Copies sdwis state lab id from the editable widget to the manager
     */
    public void copyToSample() {
        ...
        sm = parentScreen.getManager();
        labId = editableWidget.getValue();
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(labId, sm.getSampleSDWIS().getStateLabId())) {
            sm.getSampleSDWIS().setStateLabId(labId);
            nonEditableWidget.setValue(labId);
            matchImage.setResource(OpenELISResources.INSTANCE.commit());
     ...
     * sample's sdwis state lab id; increments the number of times the value has
     * been changed; if the values are different and the value has been changed
     * more than once, shows the sample's sdwis state lab id to the user
     */
