...
/**
 * 
 * This class is used to carry the data entered on QcList screen to the
...
public class QcChartScreen extends Screen {
    ...
    private void initialize() {
        ...
        addScreenHandler(qcName, new ScreenEventHandler<String>() {
            ...
            public void onStateChange(StateChangeEvent<State> event) {
                qcName.enable(EnumSet.of(State.DEFAULT)
        ...
        addScreenHandler(workSheetCreatedDateFrom, new ScreenEventHandler<Datetime>() {
            ...
            public void onStateChange(StateChangeEvent<State> event) {
                workSheetCreatedDateFrom.enable(EnumSet.of(State.DEFAULT).contains(event.getState()));
            }
        });
        ...
        plotDataTable.addBeforeCellEditedHandler(new BeforeCellEditedHandler() {            
            public void onBeforeCellEdited(BeforeCellEditedEvent event) {
                ...
                //if the row has corrupt values just display the value to the user. Also except the checkbox all other columns shoudl be non editable.
        ...
        if ( !fromEmpty) {
            if (toEmpty) {
                ...
                valid = false;
                    ...
                    workSheetCreatedDateTo.addException(new LocalizedException("requiredEitherFields"));
                    workSheetCreatedDateFrom.addException(new LocalizedException("requiredEitherFields"));
                    numInstances.addException(new LocalizedException("requiredEitherFields"));
                    valid = false;
                }
        } else if ( !toEmpty) {
    ...
    private ArrayList<TableDataRow> getTableModel() {
        ...
        for (int i = 0; i < results.getQcList().size(); i++ ) {
            ...
            tr = new TableDataRow(null,data.getIsPlot(),
