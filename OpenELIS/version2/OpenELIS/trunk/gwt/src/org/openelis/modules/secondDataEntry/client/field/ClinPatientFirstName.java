...
/**
 * This class manages the widgets used for verifying clinical patient first name
 */
public class ClinPatientFirstName extends SingleField<TextBox<String>> {
    public ClinPatientFirstName(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    protected void init() {
        ...
        key = SampleMeta.CLIN_PATIENT_FIRST_NAME;
    ...
    /**
     * Copies clinical patient first name from the manager to the editable
     * widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleClinical()
                                                                  .getPatient()
                                                                  .getFirstName())) {
            editableWidget.setValue(sm.getSampleClinical().getPatient().getFirstName());
    ...
    /**
     * Copies clinical patient first name from the editable widget to the
     * manager
     */
    public void copyToSample() {
        ...
        sm = parentScreen.getManager();
        fname = editableWidget.getValue();
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(fname, sm.getSampleClinical().getPatient().getFirstName())) {
            sm.getSampleClinical().getPatient().setFirstName(fname);
            nonEditableWidget.setValue(fname);
            matchImage.setResource(OpenELISResources.INSTANCE.commit());
     ...
     * sample's clinical patient first name; increments the number of times the
     * value has been changed; if the values are different and the value has
     * been changed more than once, shows the sample's clinical patient first
     * name to the user
     */
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's clinical patient first name
     */
    public void verify() {
