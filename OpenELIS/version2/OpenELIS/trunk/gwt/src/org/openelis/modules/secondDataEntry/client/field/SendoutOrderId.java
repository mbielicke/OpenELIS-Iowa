...
public class SendoutOrderId extends SingleField<TextBox<Integer>> {
    ...
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSample().getOrderId())) {
            editableWidget.setValue(sm.getSample().getOrderId());
     ...
     * send-out order id; otherwise shows an error
     */
    public void copyToSample() {
        Integer accession, orderId;
        SampleManager1 sm;
        IOrderManager1 om;

        sm = parentScreen.getManager();
        orderId = editableWidget.getValue();
        if (numEdit <= 1 || editableWidget.hasExceptions() ||
            !DataBaseUtil.isDifferent(orderId, sm.getSample().getOrderId()))
            return;
        ...
        if (orderId != null) {
            ...
            if (om == null || !Constants.iorder().SEND_OUT.equals(om.getIorder().getType())) {
                accession = sm.getSample().getAccessionNumber();
                Window.alert(Messages.get()
                                     .sample_orderIdInvalidException(accession != null ? accession
                                                                                      : 0,
                                                                     orderId));
                return;
            }
        }
        sm.getSample().setOrderId(orderId);
        nonEditableWidget.setValue(orderId);
        matchImage.setResource(OpenELISResources.INSTANCE.commit());
     ...
     * sample's order id; increments the number of times the value has been
     * changed; if the values are different and the value has been changed more
     * than once, shows the sample's order id to the user
     */
     ...
     */
    public void verify() {
        isVerified = !DataBaseUtil.isDifferent(editableWidget.getValue(), parentScreen.getManager()
                                                                                      .getSample()
                                                                                      .getOrderId());
