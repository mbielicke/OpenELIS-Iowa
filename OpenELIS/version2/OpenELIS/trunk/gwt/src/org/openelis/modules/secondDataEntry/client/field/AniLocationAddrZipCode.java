 ...
 * address zip code
 */
public class AniLocationAddrZipCode extends SingleField<TextBox<String>> {
    public AniLocationAddrZipCode(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    protected void init() {
        ...
        key = SampleMeta.ANI_LOCATION_ADDR_ZIP_CODE;
    ...
    /**
     * Copies animal location address zip code from the manager to the
     * editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleAnimal()
                                                                  .getLocationAddress()
                                                                  .getZipCode())) {
            editableWidget.setValue(sm.getSampleAnimal().getLocationAddress().getZipCode());
    ...
    /**
     * Copies animal location address zip code from the editable widget
     * to the manager
     */
    public void copyToSample() {
        ...
        if (numEdit > 1 &&
            ...
            DataBaseUtil.isDifferent(zip, sm.getSampleAnimal()
                                            .getLocationAddress()
                                            .getZipCode())) {
            sm.getSampleAnimal().getLocationAddress().setZipCode(zip);
     ...
     * sample's animal location address zip code; increments the number
     * of times the value has been changed; if the values are different and the
     * value has been changed more than once, shows the sample's animal
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's animal location address zip code
     */
    public void verify() {
