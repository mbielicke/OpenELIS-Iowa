...
/**
 * This class manages the widgets used for verifying sdwis sample point id
 */
public class SDWISSamplePointId extends SingleField<TextBox<String>> {
    public SDWISSamplePointId(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    protected void init() {
        ...
        key = SampleMeta.SDWIS_SAMPLE_POINT_ID;
    ...
    /**
     * Copies sdwis sample point id from the manager to the editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleSDWIS()
                                                                  .getSamplePointId())) {
            editableWidget.setValue(sm.getSampleSDWIS().getSamplePointId());
    ...
    /**
     * Copies sdwis sample point id from the editable widget to the manager
     */
    public void copyToSample() {
        ...
        sm = parentScreen.getManager();
        pointId = editableWidget.getValue();
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(pointId, sm.getSampleSDWIS().getSamplePointId())) {
            sm.getSampleSDWIS().setSamplePointId(pointId);
            nonEditableWidget.setValue(pointId);
            matchImage.setResource(OpenELISResources.INSTANCE.commit());
     ...
     * sample's sdwis sample point id; increments the number of times the value
     * has been changed; if the values are different and the value has been
     * changed more than once, shows the sample's sdwis sample point id to the
     * user
     */
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's sdwis sample point id
     */
    public void verify() {
