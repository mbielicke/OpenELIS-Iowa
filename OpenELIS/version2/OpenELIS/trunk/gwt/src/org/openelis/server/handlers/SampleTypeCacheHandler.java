...
public class SampleTypeCacheHandler implements MessageHandler<SampleTypeCacheMessage> {
    
    static CategoryRemote remote = (CategoryRemote)EJBFactory.lookup("openelis/CategoryBean/remote");
    
    public static int version = 0;

    public void handle(SampleTypeCacheMessage message) {
        CachingManager.remove("InitialData", "sampleTypeDropdown");
        
    }
    
    public static TableDataModel<TableDataRow<Integer>> getSampleTypes() {
        TableDataModel<TableDataRow<Integer>> model = (TableDataModel<TableDataRow<Integer>>)CachingManager.getElement("InitialData", "sampleTypeDropdown");
        if(model == null) {
            ...
            CachingManager.putElement("InitialData", "sampleTypeDropdown", model);
            version++;
        }
        return model;
