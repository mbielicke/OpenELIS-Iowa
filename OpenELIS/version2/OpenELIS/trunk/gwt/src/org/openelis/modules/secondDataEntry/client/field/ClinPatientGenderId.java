...
/**
 * This class manages the widgets used for verifying clinical patient gender id
 */
public class ClinPatientGenderId extends SingleField<Dropdown<Integer>> {
    public ClinPatientGenderId(VerificationScreen parentScreen, TableRowElement tableRowElement,
                               Dropdown<Integer> editableWidget,
                               Dropdown<Integer> nonEditableWidget, Image matchImage,
    ...
    protected void init() {
        ...
        key = SampleMeta.CLIN_PATIENT_GENDER_ID;
        ...
        parentScreen.getEventBus().addHandler(PatientChangeEvent.getType(),
                                              new PatientChangeEvent.Handler() {
                                                  ...
                                                  public void onPatientChange(PatientChangeEvent event) {
                                                       ...
                                                       * new patient's gender;
                                                       * if the previous
                                                       * patient's gender was
    ...
    /**
     * Copies clinical patient gender id from the manager to the editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleClinical()
                                                                  .getPatient()
                                                                  .getGenderId())) {
            editableWidget.setValue(sm.getSampleClinical().getPatient().getGenderId());
    ...
    /**
     * Copies clinical patient gender id from the editable widget to the manager
     */
    public void copyToSample() {
        ...
        sm = parentScreen.getManager();
        genId = editableWidget.getValue();
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(genId, sm.getSampleClinical().getPatient().getGenderId())) {
            sm.getSampleClinical().getPatient().setGenderId(genId);
            nonEditableWidget.setValue(genId);
            matchImage.setResource(OpenELISResources.INSTANCE.commit());
     ...
     * sample's clinical patient gender id; increments the number of times the
     * value has been changed; if the values are different and the value has
     * been changed more than once, shows the sample's clinical patient gender
     * to the user
     */
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's clinical patient gender id
     */
    public void verify() {
