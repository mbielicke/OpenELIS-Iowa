 ...
 */
public class EnvCollector extends SingleField<TextBox<String>> {
    public EnvCollector(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    /**
     * Copies environmental collector from the manager to the editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleEnvironmental()
                                                                  .getCollector())) {
            editableWidget.setValue(sm.getSampleEnvironmental().getCollector());
    ...
    /**
     * Copies environmental collector from the editable widget to the manager
     */
    public void copyToSample() {
        ...
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(col, sm.getSampleEnvironmental().getCollector())) {
            sm.getSampleEnvironmental().setCollector(col);
     ...
     * sample's environmental collector; increments the number of times the
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's environmental collector
     */
    public void verify() {
