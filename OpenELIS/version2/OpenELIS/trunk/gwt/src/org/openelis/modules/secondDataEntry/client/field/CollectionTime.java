...
/**
 * This class manages the widgets used for verifying collection time
 */
public class CollectionTime extends SingleField<Calendar> {
    public CollectionTime(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    protected void init() {
        ...
        key = SampleMeta.COLLECTION_TIME;
    ...
    /**
     * Copies collection time from the manager to the editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferentDT(editableWidget.getValue(), sm.getSample()
                                                                    .getCollectionTime())) {
            editableWidget.setValue(sm.getSample().getCollectionTime());
    ...
    /**
     * Copies collection time from the editable widget to the manager
     */
    public void copyToSample() {
        ...
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferentDT(cdt, sm.getSample().getCollectionTime())) {
            sm.getSample().setCollectionTime(cdt);
     ...
     * sample's collection time; increments the number of times the value has
     * been changed; if the values are different and the value has been changed
     * more than once, shows the sample's collection time to the user
     */
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's collection time
     */
    public void verify() {
        isVerified = !DataBaseUtil.isDifferentDT(editableWidget.getValue(),
