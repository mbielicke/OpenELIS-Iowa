...
public class ReportScreen extends Screen {
    ...
    protected String            name, attachmentName, runReportInterface, getPromptsInterface;
    ...
    /**
     * Gets/sets the attachment filename for files that are returned to the browser
     */
        ...
        for (i = 0; i < reportParameters.size(); i++ ) {
            ...
            switch (p.getType()) {
                ...
                case CHECK:
                    ...
                    break;
                case STRING:
                    w = createTextBox("STRING", p);
                    ((TextBox)w).setEnabled(true);
                case SHORT:
                case INTEGER:
                    w = createTextBox("INTEGER", p);
                    ((TextBox)w).setEnabled(true);
                    ((TextBox)w).setHelper(new IntegerHelper());
                    break;
                case FLOAT:
                case DOUBLE:
                    w = createTextBox("DOUBLE", p);
                    ((TextBox)w).setEnabled(true);
                    ((TextBox)w).setHelper(new DoubleHelper());
                    break;
                case DATETIME:
                    w = createTextBox("DATE", p);
                    DateHelper helper = new DateHelper();
                    helper.setBegin(getDatetimeCode(p.getDatetimeStartCode()));
                    helper.setEnd(getDatetimeCode(p.getDatetimeEndCode()));
                    ((TextBox)w).setEnabled(true);
                    ((TextBox)w).setHelper(helper);
                    break;
                default:
                    w = null;
    ...
    protected void runReport() {
        ...
        service.call(runReportInterface, query, new AsyncCallback<ReportStatus>() {
     ...
     */
    protected void resetDropdown(Prompt p, ArrayList<Item<String>> l, Dropdown<String> d) {
        String key;
    ...
    protected QueryData getQuery(TextBox tb, String key) {
        ...
        qd.setKey(key);
        helper = tb.getHelper();
        if (helper instanceof StringHelper)
            qd.setType(QueryData.Type.STRING);
        else if (helper instanceof IntegerHelper)
            qd.setType(QueryData.Type.INTEGER);
        else if (helper instanceof DoubleHelper)
            qd.setType(QueryData.Type.DOUBLE);
        else if (helper instanceof DateHelper)
    ...
    protected TextBox<?> createTextBox(String f, Prompt p) {
        TextBox<?> t;
