...
public class TestSectionFlagsCacheHandler implements
                                         MessageHandler<TestSectionFlagsCacheMessage> {

    static CategoryRemote remote = (CategoryRemote)EJBFactory.lookup("openelis/CategoryBean/remote");
    
    public static int version = 0;
    
    public void handle(TestSectionFlagsCacheMessage message) {
        CachingManager.remove("InitialData", "testSectionFlagDropdown");                
    }
    
    public static TableDataModel<TableDataRow<Integer>> getTestSections() {
        TableDataModel<TableDataRow<Integer>> model = (TableDataModel<TableDataRow<Integer>>)CachingManager.getElement("InitialData", "testSectionFlagDropdown");
        TableDataRow<Integer> row = null;
        if(model == null) {
            CategoryRemote remote = (CategoryRemote)EJBFactory.lookup("openelis/CategoryBean/remote");        
            Integer id = remote.getCategoryId("test_section_flags");
            ...
            CachingManager.putElement("InitialData", "testSectionFlagDropdown", model);
            version++;
        }
        return model;
