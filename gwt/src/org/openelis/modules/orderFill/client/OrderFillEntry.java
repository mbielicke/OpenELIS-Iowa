...
public class OrderFillEntry implements EntryPoint {
    
    @Override
    public void onModuleLoad() {
        OpenELISEntry.mainBus.addHandler(OpenELIS.ORDER_FILL, new ShowScreenHandler() {
                ...
                GWT.runAsync(new RunAsyncCallback() {
                    public void onSuccess() {
                        try {
                            ...
                            window.setName(OpenELIS.getMessages().fillOrder());
                            window.setSize("20px", "20px");
                            window.setContent(new OrderFillScreen(window));
                            OpenELIS.getBrowser().addWindow(window, "fillOrder");
                        } catch (Throwable e) {
