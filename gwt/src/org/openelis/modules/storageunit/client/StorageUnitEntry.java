...
public class StorageUnitEntry implements EntryPoint {
    
    @Override
    public void onModuleLoad() {
        OpenELISEntry.mainBus.addHandler(OpenELIS.STORAGE_UNIT, new ShowScreenHandler() {
                ...
                GWT.runAsync(new RunAsyncCallback() {
                    public void onSuccess() {
                        try {
                            ...
                            window.setName(Messages.get().storageUnit());
                            window.setSize("20px", "20px");
                            window.setContent(new StorageUnitScreen(window));
                            OpenELIS.getBrowser().addWindow(window, "storageUnit");
                        } catch (Throwable e) {
