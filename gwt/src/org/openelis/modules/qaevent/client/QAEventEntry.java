...
public class QAEventEntry implements EntryPoint {
    
    @Override
    public void onModuleLoad() {
        OpenELISEntry.mainBus.addHandler(OpenELIS.QA_EVENT, new ShowScreenHandler() {
                ...
                GWT.runAsync(new RunAsyncCallback() {
                    public void onSuccess() {
                        try {
                            ...
                            window.setName(OpenELIS.getMessages().QAEvent());
                            window.setSize("20px", "20px");
                            window.setContent(new QaEventScreen(window));
                            OpenELIS.getBrowser().addWindow(window, "QAEvent");
                        } catch (Throwable e) {
