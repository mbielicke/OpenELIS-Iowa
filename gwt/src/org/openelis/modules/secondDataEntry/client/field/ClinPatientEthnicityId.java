...
/**
 * This class manages the widgets used for verifying clinical patient ethnicity
 * id
 */
public class ClinPatientEthnicityId extends SingleField<Dropdown<Integer>> {
    public ClinPatientEthnicityId(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    protected void init() {
        ...
        key = SampleMeta.CLIN_PATIENT_ETHNICITY_ID;
    ...
    /**
     * Copies clinical patient ethnicity id from the manager to the editable
     * widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleClinical()
                                                                  .getPatient()
                                                                  .getEthnicityId())) {
            editableWidget.setValue(sm.getSampleClinical().getPatient().getEthnicityId());
    ...
    /**
     * Copies clinical patient ethnicity id from the editable widget to the
     * manager
     */
    public void copyToSample() {
        ...
        sm = parentScreen.getManager();
        ethId = editableWidget.getValue();
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(ethId, sm.getSampleClinical().getPatient().getEthnicityId())) {
            sm.getSampleClinical().getPatient().setEthnicityId(ethId);
            nonEditableWidget.setValue(ethId);
            matchImage.setResource(OpenELISResources.INSTANCE.commit());
     ...
     * sample's clinical patient ethnicity id; increments the number of times
     * the value has been changed; if the values are different and the value has
     * been changed more than once, shows the sample's clinical patient
     * ethnicity to the user
     */
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's clinical patient ethnicity id
     */
    public void verify() {
