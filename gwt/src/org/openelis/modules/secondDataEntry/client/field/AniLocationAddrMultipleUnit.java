...
/**
 * This class manages the widgets used for verifying animal location
 * address multiple unit
 */
public class AniLocationAddrMultipleUnit extends SingleField<TextBox<String>> {
    public AniLocationAddrMultipleUnit(VerificationScreen parentScreen,
    ...
    protected void init() {
        ...
        key = SampleMeta.ANI_LOCATION_ADDR_MULTIPLE_UNIT;
    ...
    /**
     * Copies animal location address multiple unit from the manager to
     * the editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleAnimal()
                                                                  .getLocationAddress()
                                                                  .getMultipleUnit())) {
            editableWidget.setValue(sm.getSampleAnimal()
                                      .getLocationAddress()
                                      .getMultipleUnit());
    ...
    /**
     * Copies animal location address multiple unit from the editable
    ...
    public void copyToSample() {
        ...
        if (numEdit > 1 &&
            ...
            DataBaseUtil.isDifferent(multi, sm.getSampleAnimal()
                                              .getLocationAddress()
                                              .getMultipleUnit())) {
            sm.getSampleAnimal().getLocationAddress().setMultipleUnit(multi);
     ...
     * sample's animal location address multiple unit; increments the
     ...
     * animal location address multiple unit to the user
     */
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's animal location address multiple unit
     */
    public void verify() {
