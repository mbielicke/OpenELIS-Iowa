...
/**
 * This class manages the widgets used for verifying sdwis sample type id
 */
public class SDWISSampleTypeId extends SingleField<Dropdown<Integer>> {
    public SDWISSampleTypeId(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    protected void init() {
        ...
        setRowVisible();
        key = SampleMeta.SDWIS_SAMPLE_TYPE_ID;
    ...
    /**
     * Copies sdwis sample type id from the manager to the editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleSDWIS()
                                                                  .getSampleTypeId())) {
            editableWidget.setValue(sm.getSampleSDWIS().getSampleTypeId());
    ...
    /**
     * Copies sdwis sample type id from the editable widget to the manager
     */
    public void copyToSample() {
        ...
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(typeId, sm.getSampleSDWIS().getSampleTypeId())) {
            sm.getSampleSDWIS().setSampleTypeId(typeId);
     ...
     * sample's sdwis sample type id; increments the number of times the value
     * has been changed; if the values are different and the value has been
     * changed more than once, shows the sample's sdwis sample type id to the
     * user
     */
