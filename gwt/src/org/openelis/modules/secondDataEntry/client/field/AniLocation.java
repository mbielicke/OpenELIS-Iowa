 ...
 */
public class AniLocation extends SingleField<TextBox<String>> {
    public AniLocation(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    /**
     * Copies animal location from the manager to the editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleAnimal()
                                                                  .getLocation())) {
            editableWidget.setValue(sm.getSampleAnimal().getLocation());
    ...
    /**
     * Copies animal location from the editable widget to the manager
     */
    public void copyToSample() {
        ...
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(loc, sm.getSampleAnimal().getLocation())) {
            sm.getSampleAnimal().setLocation(loc);
     ...
     * sample's animal location; increments the number of times the value
     * has been changed; if the values are different and the value has been
     * changed more than once, shows the sample's animal location to the
     * user
     */
