 ...
 * address state
 */
public class AniLocationAddrState extends SingleField<Dropdown<String>> {
    public AniLocationAddrState(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    protected void init() {
        ...
        key = SampleMeta.ANI_LOCATION_ADDR_STATE;
    ...
    /**
     * Copies animal location address state from the manager to the
     * editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleAnimal()
                                                                  .getLocationAddress()
                                                                  .getState())) {
            editableWidget.setValue(sm.getSampleAnimal().getLocationAddress().getState());
    ...
    /**
     * Copies animal location address state from the editable widget to
     * the manager
     */
    public void copyToSample() {
        ...
        if (numEdit > 1 &&
            ...
            DataBaseUtil.isDifferent(state, sm.getSampleAnimal()
                                              .getLocationAddress()
                                              .getState())) {
            sm.getSampleAnimal().getLocationAddress().setState(state);
     ...
     * sample's animal location address state; increments the number of
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's animal location address state
     */
    public void verify() {
