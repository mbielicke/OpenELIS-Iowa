 ...
 * multiple unit
 */
public class ClinPatientAddrMultipleUnit extends SingleField<TextBox<String>> {
    public ClinPatientAddrMultipleUnit(VerificationScreen parentScreen,
    ...
    protected void init() {
        ...
        key = SampleMeta.CLIN_PATIENT_ADDR_MULTIPLE_UNIT;
        ...
        parentScreen.getEventBus().addHandler(PatientChangeEvent.getType(),
                                              new PatientChangeEvent.Handler() {
                                                  ...
                                                  public void onPatientChange(PatientChangeEvent event) {
                                                       ...
                                                       * new patient's multiple
                                                       * unit; if the previous
                                                       * patient's city was
                                                       * being shown in the non
                                                       * editable widget,
                                                       * replace that with the
                                                       * new patient's multiple
                                                       * unit; if no value was
    ...
    /**
     * Copies clinical patient address multiple unit from the manager to the
     * editable widget
     */
    ...
    /**
     * Copies clinical patient address multiple unit from the editable widget to
     * the manager
     */
    public void copyToSample() {
        ...
        if (numEdit > 1 &&
            ...
            DataBaseUtil.isDifferent(multi, sm.getSampleClinical()
                                              .getPatient()
                                              .getAddress()
                                              .getMultipleUnit())) {
            sm.getSampleClinical().getPatient().getAddress().setMultipleUnit(multi);
     ...
     * sample's clinical patient address multiple unit; increments the number of
     ...
     * patient address multiple unit to the user
     */
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's clinical patient address multiple unit
     */
    public void verify() {
