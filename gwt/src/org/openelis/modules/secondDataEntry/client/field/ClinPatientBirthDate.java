...
/**
 * This class manages the widgets used for verifying clinical patient birth date
 */
public class ClinPatientBirthDate extends SingleField<Calendar> {
    public ClinPatientBirthDate(VerificationScreen parentScreen, TableRowElement tableRowElement,
                                Calendar editableWidget, Calendar nonEditableWidget,
    ...
    protected void init() {
        ...
        key = SampleMeta.CLIN_PATIENT_BIRTH_DATE;
        ...
        parentScreen.getEventBus().addHandler(PatientChangeEvent.getType(),
                                              new PatientChangeEvent.Handler() {
                                                  ...
                                                  public void onPatientChange(PatientChangeEvent event) {
                                                       ...
                                                       * new patient's birth
                                                       * date; if the previous
                                                       * patient's birth date
                                                       * was being shown in the
                                                       * non editable widget,
                                                       * replace that with the
                                                       * new patient's birth
                                                       * date; if no value was
    ...
    /**
     * Copies clinical patient birth date from the manager to the editable
     * widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferentYD(editableWidget.getValue(), sm.getSampleClinical()
                                                                    .getPatient()
                                                                    .getBirthDate())) {
            editableWidget.setValue(sm.getSampleClinical().getPatient().getBirthDate());
    ...
    /**
     * Copies clinical patient birth date from the editable widget to the
     * manager
     */
    public void copyToSample() {
        ...
        sm = parentScreen.getManager();
        bdt = editableWidget.getValue();
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferentYD(bdt, sm.getSampleClinical().getPatient().getBirthDate())) {
            sm.getSampleClinical().getPatient().setBirthDate(bdt);
            nonEditableWidget.setValue(bdt);
            matchImage.setResource(OpenELISResources.INSTANCE.commit());
     ...
     * sample's clinical patient birth date increments the number of times the
     * value has been changed; if the values are different and the value has
     * been changed more than once, shows the sample's clinical patient birth
     * date to the user
     */
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's clinical patient birth date
     */
    public void verify() {
        isVerified = !DataBaseUtil.isDifferentYD(editableWidget.getValue(),
