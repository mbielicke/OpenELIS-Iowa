...
/**
 * This class manages the widgets used for verifying sdwis location
 */
public class SDWISLocation extends SingleField<TextBox<String>> {
    public SDWISLocation(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    /**
     * Copies sdwis location from the manager to the editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleSDWIS().getLocation())) {
            editableWidget.setValue(sm.getSampleSDWIS().getLocation());
    ...
    /**
     * Copies sdwis location from the editable widget to the manager
     */
    public void copyToSample() {
        ...
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(loc, sm.getSampleSDWIS().getLocation())) {
            sm.getSampleSDWIS().setLocation(loc);
     ...
     * sample's sdwis locationr; increments the number of times the value has
     * been changed; if the values are different and the value has been changed
     * more than once, shows the sample's sdwis location to the user
     */
    public void valueChanged() {
        ...
        if ( !isVerified) {
            ...
            if (numEdit > 1)
                nonEditableWidget.setValue(parentScreen.getManager().getSampleSDWIS().getLocation());
            /*
