...
/**
 * This class manages the widgets used for verifying animal common name id
 */
public class AniAnimalCommonNameId extends SingleField<Dropdown<Integer>> {
    public AniAnimalCommonNameId(VerificationScreen parentScreen,
    ...
    protected void init() {
        ...
        setRowVisible();
        key = SampleMeta.ANI_ANIMAL_COMMON_NAME_ID;
    ...
    /**
     * Copies animal common name id from the manager to the editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleAnimal()
                                                                  .getAnimalCommonNameId())) {
            editableWidget.setValue(sm.getSampleAnimal().getAnimalCommonNameId());
    ...
    /**
     * Copies animal common name id from the editable widget to the manager
     */
    public void copyToSample() {
        ...
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(typeId, sm.getSampleAnimal().getAnimalCommonNameId())) {
            sm.getSampleAnimal().setAnimalCommonNameId(typeId);
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's animal common name id
     */
    public void verify() {
