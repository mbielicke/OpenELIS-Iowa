...
/**
 * This class manages the widgets used for verifying received by id
 */
public class ReceivedById extends SingleField<AutoComplete> {
    public ReceivedById(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    /**
     * Copies the received by id from the manager to the editable widget
     */
    public void copyFromSample() {
        Integer userId;
        SampleManager1 sm;

        userId = editableWidget.getValue() != null ? editableWidget.getValue().getId() : null;
        sm = parentScreen.getManager();
        if (numEdit > 1 && DataBaseUtil.isDifferent(userId, sm.getSample().getReceivedById())) {
            try {
                editableWidget.setValue(sm.getSample().getReceivedById(), getReceivedByName());
    ...
    /**
     * Copies received by id from the editable widget to the manager
     */
    public void copyToSample() {
        ...
        user = editableWidget.getValue() != null ? (SystemUserVO)editableWidget.getValue()
                                                                               .getData() : null;
        userId = user != null ? user.getId() : null;
        sm = parentScreen.getManager();
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(userId, sm.getSample().getReceivedById())) {
            sm.getSample().setReceivedById(userId);
     ...
     * sample's received by id; increments the number of times the value has
     * been changed; if the values are different and the value has been changed
     * more than once, shows the sample's received by user's login name to the
     * user
     */
            ...
            if (numEdit > 1) {
                try {
                    nonEditableWidget.setValue(parentScreen.getManager()
                                                           .getSample()
                                                           .getReceivedById(), getReceivedByName());
                } catch (Exception e) {
                    Window.alert(e.toString());
                    logger.log(Level.SEVERE, e.getMessage(), e);
                }
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's received by id
     */
    public void verify() {
        Integer userId;

        userId = editableWidget.getValue() != null ? editableWidget.getValue().getId() : null;
        isVerified = !DataBaseUtil.isDifferent(userId, parentScreen.getManager()
                                                                   .getSample()
                                                                   .getReceivedById());
     ...
     * received by id is null
     */
    private String getReceivedByName() throws Exception {
        SampleManager1 sm;

        sm = parentScreen.getManager();
        if (sm.getSample().getReceivedById() == null)
            return null;
        return UserCache.getSystemUser(sm.getSample().getReceivedById()).getLoginName();
    }
}