...
/**
 * This class manages the widgets used for verifying animal provider phone
 */
public class AniProviderPhone extends SingleField<TextBox<String>> {
    public AniProviderPhone(VerificationScreen parentScreen, TableRowElement tableRowElement,
    ...
    protected void init() {
        ...
        key = SampleMeta.ANI_PROVIDER_PHONE;
    ...
    /**
     * Copies animal provider phone from the manager to the editable widget
     */
    public void copyFromSample() {
        ...
        if (numEdit > 1 &&
            DataBaseUtil.isDifferent(editableWidget.getValue(), sm.getSampleAnimal()
                                                                  .getProviderPhone())) {
            editableWidget.setValue(sm.getSampleAnimal().getProviderPhone());
    ...
    /**
     * Copies animal provider phone from the editable widget to the manager
     */
    public void copyToSample() {
        ...
        if (numEdit > 1 && !editableWidget.hasExceptions() &&
            DataBaseUtil.isDifferent(phone, sm.getSampleAnimal().getProviderPhone())) {
            sm.getSampleAnimal().setProviderPhone(phone);
     ...
     * sample's animal provider phone; increments the number of times the
     * value has been changed; if the values are different and the value has
     * been changed more than once, shows the sample's animal provider phone
     * to the user
     */
    ...
    /**
     * Verifies whether the value entered by the user is the same as the
     * sample's animal provider phone
     */
    public void verify() {
