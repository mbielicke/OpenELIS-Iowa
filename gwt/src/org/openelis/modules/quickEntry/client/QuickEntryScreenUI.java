...
public class QuickEntryScreenUI extends Screen implements CacheProvider {
    ...
    @UiField
    protected Table<Row>                                   quickEntryTable;
    @UiField
    ...
    public void initialize() throws Exception {
        ...
        addScreenHandler(testMethodSampleType, "testMethodSampleType", new ScreenHandler<String>() {
            public void onValueChange(ValueChangeEvent<String> event) {
                ...
                if (testMethodSampleType.getSelectedItem() != null) {
                    typeDO = (TestMethodSampleTypeVO)testMethodSampleType.getSelectedItem()
                                                                         .getData();
                    if (typeDO.getTestId() != null) {
                        try {
                            sections = testSectionHash.get(typeDO.getTestId());
                            if (sections == null) {
                                sections = TestService.get()
                                                      .fetchTestSectionsByTestId(typeDO.getTestId());
                            ...
                            for (TestSectionViewDO tsVDO : sections) {
                                ...
                                item = (new Item<Integer>(tsVDO.getSectionId(), tsVDO.getSection()));
                                item.setData(tsVDO);
                                model.add(item);
                            }
                        ...
                        try {
                            ...
                            for (IdVO testVO : testIds) {
                                ...
                                if (sections == null) {
                                    sections = TestService.get()
                                                          .fetchTestSectionsByTestId(testVO.getId());
                                    testSectionHash.put(typeDO.getTestId(), sections);
                                }
                                for (TestSectionViewDO tsVDO : sections) {
        ...
        addScreenHandler(quickEntryTable,
                         "quickEntryTable",
                         new ScreenHandler<ArrayList<Item<String>>>() {
                             public void onDataChange(DataChangeEvent event) {
        ...
        receivedBy.setValue(UserCache.getPermission().getSystemUserId(), UserCache.getPermission()
                                                                                  .getLoginName());
                    ...
                    public void onSelection(SelectionEvent<Integer> event) {
                        switch (event.getSelectedItem().intValue()) {
                            case 0:
                                ...
                                break;
                            case 1:
                                receivedDate.setValue(new Datetime(Datetime.YEAR,
                                                                   Datetime.MINUTE,
                                                                   recDate));
                                break;
                        }
            ...
            if (val.matches("[TP][0-9]*\\-[0-9]*")) { // test & panel
                ...
                if (validateFields()) {
                    ...
                    try {
                        ...
                        if (smRowCount == null) {
                            try {
                                sampleMan = SampleService1.get()
                                                          .fetchByAccession(accessionNum,
                                                                            (SampleManager1.Load[])null);
                                if (Constants.dictionary().SAMPLE_RELEASED.equals(sampleMan.getSample()
                                                                                           .getStatusId()))
                                    throw new InconsistencyException(Messages.get()
                                                                             .sample_cantAddAnalysis());
                                sampleMan = SampleService1.get()
                                                          .fetchForUpdate(sampleMan.getSample()
                                                                                   .getId(),
                                                                          (SampleManager1.Load[])null);
            ...
            if (testSelectionLookup == null) {
                testSelectionLookup = new TestSelectionLookupUI() {
                    ...
                    public void ok() {
                        ...
                        /*
                         * to set the focus back on the entry widget, a command
                         * must be scheduled
                         */
                        Scheduler.get().scheduleDeferred(focusEntryCmd);
                    }
            ...
            if (adVDO.getValue() != null && adVDO.getTypeId() == null) {
                try {
                    ...
                    errors.add(new Exception(Messages.get()
                                                     .aux_defaultValueInvalidException(man.getSample()
                                                                                          .getAccessionNumber(),
                        ...
                        for (l = 0; l < man.result.count(aVDO, k); l++ ) {
                            ...
                            if (rVDO.getValue() != null && rVDO.getTypeId() == null) {
                                try {
                                    ...
                                    errors.add(new Exception(Messages.get()
                                                                     .result_defaultValueInvalidException(man.getSample()
                                                                                                             .getAccessionNumber(),
                                                                                                          aVDO.getTestName(),
                                                                                                          aVDO.getMethodName(),
                                                                                                          rVDO.getAnalyte(),
