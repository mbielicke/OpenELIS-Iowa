...
public class AuxDataUtil {
    ...
    public static ValidationErrorsList addAuxGroupsFromPanel(Integer panelId, AuxDataManager auxMan) throws Exception {
        ...
        auxIds = PanelService1Impl.INSTANCE.fetchAuxIdsByPanelId(panelId);

        return addAuxGroupsFromAuxIds(auxIds, auxMan);
    }

    public static ValidationErrorsList addAuxGroupsFromAuxIds(ArrayList<IdVO> auxIds,
                                                              AuxDataManager auxMan) throws Exception {
        int i;
    ...
    public static ValidationErrorsList addAuxGroupsFromAuxFields(ArrayList<AuxFieldManager> fields,
                                                                 AuxDataManager auxMan) throws Exception {
        int i;
        ValidationErrorsList errors;
    ...
    private static void addAuxDataFromAuxField(AuxFieldManager fieldMan, AuxDataManager auxMan,
        ...
        if (count > 0) {
            ...
            /*
             * check to see if these fields have already been added to the
             * AuxDataManager
             */
            if (groupAddedToParent(field.getAuxFieldGroupId(), auxMan)) {
                errors.add(new FormErrorWarning(Messages.get()
                                                        .auxGrpAlreadyAddedException(field.getAuxFieldGroupName())));
                return;
            }
        }
            ...
            if ("Y".equals(field.getIsActive())) {
                ...
                if (defaultVal != null) {
                    try {
                        ...
                        errors.add(new FormErrorWarning(Messages.get()
                                                                .illegalDefaultValueForAuxFieldException(defaultVal.getValue(),
