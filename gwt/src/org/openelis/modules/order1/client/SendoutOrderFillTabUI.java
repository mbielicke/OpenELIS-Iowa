...
public class SendoutOrderFillTabUI extends Screen {
    ...
    private void displayFills() {
        ...
        if (count1 == count2) {
            ...
            for (int i = 0; i < count1; i++ ) {
                ...
                if (DataBaseUtil.isDifferent(fill1.getQuantity(), fill2.getQuantity()) ||
                    DataBaseUtil.isDifferent(fill1.getInventoryItemId(), fill2.getInventoryItemId())) {
                    dataChanged = true;
                    break;
                }
            }
    ...
    private ArrayList<Row> getTableModel() {
        ...
        for (i = 0; i < manager.fill.count(); i++ ) {
            data = manager.fill.get(i);
            row = new Row(5);
            row.setCell(0, data.getInventoryItemName());
            row.setCell(1, data.getStorageLocationName());
            row.setCell(2, data.getQuantity());
            row.setCell(3, data.getInventoryLocationLotNumber());
            row.setCell(4, data.getInventoryLocationExpirationDate());
            // if (FillTabLayout.VENDOR.equals(layout)) {
            // row.setCell(5, data.getInventoryReceiptReceivedDate());
            // row.setCell(6, data.getInventoryReceiptUnitCost());
            // row.setCell(7, data.getInventoryReceiptExternalReference());
            // }
            row.setData(data);
            model.add(row);
        }
