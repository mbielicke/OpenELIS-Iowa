...
public class InventoryItemEntry implements EntryPoint {
    
    @Override
    public void onModuleLoad() {
        OpenELISEntry.mainBus.addHandler(OpenELIS.INVENTORY_ITEM, new ShowScreenHandler() {
                ...
                GWT.runAsync(new RunAsyncCallback() {
                    public void onSuccess() {
                        try {
                            ...
                            window.setName(OpenELIS.getMessages().inventoryItem());
                            window.setSize("20px", "20px");
                            window.setContent(new InventoryItemScreen(window));
                            OpenELIS.getBrowser().addWindow(window, "inventoryItem");
                        } catch (Throwable e) {
