...
public class WorksheetCreationEntry implements EntryPoint {
    
    @Override
    public void onModuleLoad() {
        OpenELISEntry.mainBus.addHandler(OpenELIS.WORKSHEET_CREATION, new ShowScreenHandler() {
                ...
                GWT.runAsync(new RunAsyncCallback() {
                    public void onSuccess() {
                        try {
                            ...
                            window.setName(OpenELIS.getMessages().worksheetCreation());
                            window.setSize("20px", "20px");
                            window.setContent(new WorksheetCreationScreen(window));
                            OpenELIS.getBrowser().addWindow(window, "worksheetCreation");
                        } catch (Throwable e) {
