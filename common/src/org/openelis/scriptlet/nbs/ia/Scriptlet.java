 ...
 * of those above.
 */
package org.openelis.scriptlet.nbs.ia;
...
public class Scriptlet implements ScriptletInt<SampleSO> {
    ...
    public Scriptlet(ScriptletProxy proxy) throws Exception {
        ...
        proxy.log(Level.FINE, "Initializing NeonatalIAScriptlet1", null);
        /*
         * external ids for the analytes for interpretation
         */
        if (interpretations == null) {
            proxy.log(Level.FINE, "Creating a hashset for the analytes for interpretation", null);
        ...
        if (driedBloodSpotDict == null) {
            proxy.log(Level.FINE, "Getting the dictionary for 'dried_blood_spot'", null);
    ...
    public SampleSO run(SampleSO data) {
        ...
        proxy.log(Level.FINE, "In NeonatalIAScriptlet1.run", null);
    ...
    private void addDefaultSampleItem(SampleSO data) {
        ...
        proxy.log(Level.FINE, "Adding a sample item by default with sample type: " +
                              driedBloodSpotDict.getSystemName(), null);
    ...
    private void resetCollectionAge(SampleSO data) {
        ...
        proxy.log(Level.FINE, "Resetting collection age", null);
    ...
    private void fetchPreviousManager(SampleManager1 sm) throws Exception {
        ...
        proxy.log(Level.FINE, "Fetching samples for patient: " +
                              sm.getSampleNeonatal().getPatientId(), null);
        try {
             ...
             */
            ed = null;
            proxy.log(Level.FINE, "Finding the most recent previous sample", null);
    ...
    private void removeNormalTests(SampleSO data) throws Exception {
        ...
        proxy.log(Level.FINE, "Going through the analyses of the previous sample", null);
        ...
        if (testIds.size() > 0) {
            proxy.log(Level.FINE, "Fetching test managers", null);
            /*
             * fetch the test managers for the tests found above
             */
            tms = proxy.fetchTestManagersByIds(new ArrayList<Integer>(testIds));

            proxy.log(Level.FINE, "Going through the test results of the fetched managers", null);
         ...
         */
        proxy.log(Level.FINE, "Going through the results of the previous sample", null);
        testIds.clear();
        for (i = 0; i < previousManager.item.count(); i++ ) {
            item = previousManager.item.get(i);
            for (j = 0; j < previousManager.analysis.count(item); j++ ) {
        ...
        proxy.log(Level.FINE, "Finding analyses to remove in the current manager", null);
        /*
        ...
        proxy.log(Level.FINE, "Removing " + anaIds.size() + " analyses", null);
        /*
