 ...
 * of those above.
 */
package org.openelis.scriptlet.ms.firstInteg;
...
import org.openelis.scriptlet.ms.firstTri.Compute;
import org.openelis.scriptlet.ms.firstTri.Test;
...
/**
 * The scriptlet for "ms 1st integ (Sample 1 Maternal Integrated Screen)" test.
 * It uses a compute engine to calculate the MoMs for various tests such as NT
 * and risks and interpretations for various disorders such as Downs. The
...
public class Scriptlet implements ScriptletInt<SampleSO> {
    ...
    private static final String                            INTEGRATE_TEST_NAME = "ms integrate",
                    METHOD_NAME = "eia";
    ...
    public Scriptlet(ScriptletProxy proxy, Integer analysisId) {
        ...
        proxy.log(Level.FINE, "Initializing MS1stIntegScriptlet1", null);
        try {
            if ( !initialized) {
                initialized = true;
            ...
            proxy.log(Level.FINE, "Initialized MS1stIntegScriptlet1", null);
        } catch (Exception e) {
            initialized = false;
    ...
    public SampleSO run(SampleSO data) {
        ...
        proxy.log(Level.FINE, "In MS1stIntegScriptlet1.run", null);
        ...
        if (data.getActionBefore().contains(Action_Before.RESULT)) {
            ...
            if ( !analysisId.equals(ana.getId()))
                return data;
            /*
             * don't let the analysis be unreleased if a released 2nd tri
             * integrate analysis has this sample as its 1st tri sample
             */
            try {
                sms = fetchSecondTriSamples(sm);
                if (sms != null && sms.size() > 0) {
                    accList = new ArrayList<Integer>();
                    for (SampleManager1 sm1 : sms)
                        accList.add(sm1.getSample().getAccessionNumber());
                    data.setStatus(Status.FAILED);
                    data.addException(new FormErrorException(Messages.get()
                                                                     .analysis_cantUnreleaseMS1stIntegException(accession,
                                                                                                                ana.getTestName(),
                                                                                                                ana.getMethodName(),
                                                                                                                INTEGRATE_TEST_NAME,
                                                                                                                METHOD_NAME,
                                                                                                                DataBaseUtil.concatWithSeparator(accList,
                                                                                                                                                 ", "))));
                }
    ...
    /**
     * Fetches the sample(s) where the patient is the same as the passed sample
     * and where it has been set as the 1st trimester sample on a released 2nd
     * trimester integrated analysis
     */
    private ArrayList<SampleManager1> fetchSecondTriSamples(SampleManager1 sm) throws Exception {
        QueryData field;
        ...
        field = new QueryData();
        field.setKey(SampleMeta.getResultAnalyteExternalId());
        field.setType(QueryData.Type.STRING);
        field.setQuery("accession_1st_tri");
        fields.add(field);

        field = new QueryData();
        field.setKey(SampleMeta.getResultValue());
        field.setType(QueryData.Type.STRING);
        field.setQuery(sm.getSample().getAccessionNumber().toString());
        fields.add(field);

        return Util.fetchRelatedSamples(INTEGRATE_TEST_NAME, METHOD_NAME, sm, fields, proxy);
    }
    ...
    private void setRisks(SampleSO data, AnalysisViewDO ana) {
        ...
        proxy.log(Level.FINE, "In MS1stIntegScriptlet1.setRisks", null);
        try {
    ...
    private void setResultValues(SampleSO data, AnalysisViewDO ana) throws Exception {
         ...
         */
        gestAge = compute.getGestAge();
        setValue(getResult("gest_age"), Util.getWeeksAndDays(gestAge), rf, false, data, ana);
        /*
         * the dates between which the second trimester sample must be collected
         */
        collected = data.getManager().getSample().getCollectionDate();
        setValue(getResult("coll_2nd_sample_from"),
                 getSecondCollDate(collected, 105, gestAge),
                 rf,
                 false,
                 data,
                 ana);
        setValue(getResult("coll_2nd_sample_to"),
                 getSecondCollDate(collected, 146, gestAge),
                 rf,
                 false,
                 data,
                 ana);
...
/**
    ...
    /**
     * Returns the begin or end date of the period during which the second
     * trimester sample must be collected
     */
    private Datetime getSecondCollDate(Datetime collected, int days, Integer gestAge) {
        if (collected == null || gestAge == null)
            return null;
        return collected.add(days - gestAge);
    }
