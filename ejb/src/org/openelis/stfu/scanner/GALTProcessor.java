...
public class GALTProcessor extends Processor {
		
	SampleManager1 sm;
	AnalysisViewDO analysis;
	
	public GALTProcessor(SampleManager1 sm, AnalysisViewDO analysis) {
		this.sm = sm;
	...
	public CaseManager process() throws Exception {
		...
		if(isResultFlagged(result.getTestResultId())) {
			caseFactory = new CaseFactory(EJBFactory.getDictionaryCache().getBySystemName("galt_condition").getId());
