...
public class WorksheetManager1Bean {
    ...
    /**
     * Returns a new instance of worksheet manager with pre-initailized
     * worksheet and other structures.
     */
    ...
    /**
     * Returns a worksheet manager based on the specified query and requested
     * load elements
     */
    public ArrayList<WorksheetManager1> fetchByQuery(ArrayList<QueryData> fields, int first,
                                                     int max, WorksheetManager1.Load... elements) throws Exception {
        ArrayList<Integer> ids;
    ...
    /**
     * Adds/updates the worksheet and related records in the database. The
     * records are validated before add/update and the worksheet record must
    ...
    public WorksheetManager1 update(WorksheetManager1 wm) throws Exception {
        ...
        /*
         * the front code uses negative ids (temporary ids) to link worksheet
         * items and analysis, analysis and results. The negative ids are mapped
         * to actual database ids through several maps: imap for items, amap for
         * analysis.
         */
        ...
        if (getWorksheet(wm).getId() == null || getWorksheet(wm).getId() < 0) {
            ...
            //
            if (getWorksheet(wm).getDescription() == null ||
                getWorksheet(wm).getDescription().length() == 0) {
                ...
                for (AnalysisViewDO ana : SampleManager1Accessor.getAnalyses(sMan)) {
                    if (initAnalysisIds.contains(ana.getId())) {
                        if ( !Constants.dictionary().ANALYSIS_COMPLETED.equals(ana.getStatusId()) &&
                            ...
                            aHelper.changeAnalysisStatus(sMan,
                                                         ana.getId(),
                        ...
                        if (DataBaseUtil.isDifferent(waVDO.getUnitOfMeasureId(),
                            ...
                            aHelper.changeAnalysisUnit(sMan,
                                                       ana.getId(),
                                                       waVDO.getUnitOfMeasureId());
                        ...
                        if (DataBaseUtil.isDifferent(waVDO.getUnitOfMeasureId(),
                            ...
                            aHelper.changeAnalysisUnit(sMan,
                                                       ...
                                                       waVDO.getUnitOfMeasureId());
                        if (failedRun &&
                            (Constants.dictionary().ANALYSIS_INITIATED.equals(ana.getStatusId()) || Constants.dictionary().ANALYSIS_ERROR_INITIATED.equals(ana.getStatusId()))) {
                            aHelper.changeAnalysisStatus(sMan,
                                                         ana.getId(),
                                                         Constants.dictionary().ANALYSIS_REQUEUE);
                        } else if (voidRun &&
                                   (Constants.dictionary().ANALYSIS_INITIATED.equals(ana.getStatusId()) || Constants.dictionary().ANALYSIS_ERROR_INITIATED.equals(ana.getStatusId()))) {
                        ...
                        } else if (DataBaseUtil.isDifferent(waVDO.getStatusId(), ana.getStatusId())) {
        ...
        if ( !analysisIds.isEmpty()) {
            sMans = sampleMan.fetchByAnalyses(new ArrayList<Integer>(analysisIds),
                                              SampleManager1.Load.SINGLERESULT);
            for (SampleManager1 sMan : sMans) {
                                ...
                                if (data.getAnalysisId().equals(aVDO.getId())) {
                                    ...
                                    if (excludedIds == null) {
                                        ...
                                        try {
                                            ...
                                            throw new Exception("Error loading excluded analytes: " +
                                                                anyE.getMessage());
                                        }
    ...
    public ArrayList<ResultViewDO> fetchAnalytesByAnalysis(Integer analysisId, Integer testId) throws Exception {
        ...
        for (TestAnalyteViewDO taVDO : testAnalytes) {
            if ( !analyteIds.contains(taVDO.getAnalyteId())) {
                ...
                while ( !rVDO.getRowGroup().equals(taVDO.getRowGroup()) &&
                       rowIndex < resultAnalytes.size() - 1)
                    rVDO = (ResultViewDO)resultAnalytes.get( ++rowIndex);
                while (rVDO.getRowGroup().equals(taVDO.getRowGroup()) &&
                       rowIndex < resultAnalytes.size() - 1)
                    rVDO = (ResultViewDO)resultAnalytes.get( ++rowIndex);
    ...
    public WorksheetQcChoiceVO loadTemplate(WorksheetManager1 wm, Integer testId) {
        ...
        ArrayList<TestWorksheetItemDO> lastBothList, lastRunList, lastSubsetList, randList;
        ...
        for (i = 0; i < numSub; i++ ) {
            ...
            while (j < randList.size()) {
                ...
                pos = (int) (Math.random() * (twDO.getSubsetCapacity() - 1)) + i *
                      twDO.getSubsetCapacity();
        ...
        for (k = 0; k < lastRunList.size() && i < twDO.getTotalCapacity(); i++ ) {
            ...
            try {
                ...
                errors.add(new FormErrorException("Error loading QC for position " + (i + 1) +
                                                  "; " + anyE.getMessage()));
            }
        }
        ...
        for (WorksheetAnalysisViewDO waVDO : analyses) {
            if (waVDO.getAnalysisId() != null) {
                ...
                try {
                    ...
                    throw new Exception("Error loading results from test definition: " +
                                        anyE.getMessage());
                }
                ...
                for (i = 0; i < resultDOs.size(); i++ ) {
                    ...
                    if (excludedIds.size() == 0 ||
                        !excludedIds.contains(resultDO.getTestAnalyteId())) {
                        ...
                        if ( !toWorksheet.getFormatId().equals(fromWorksheet.getFormatId())) {
                            for (i = 0; i < 30; i++ ) {
                                ...
                                if (fromName != null) {
                                    ...
                                    if (toIndex != null)
                                        toWrVDO.setValueAt(toIndex.intValue() - 9,
                                                           fromWrVDO.getValueAt(i));
                                }
                            }
                        } else {
                            for (i = 0; i < 30; i++ )
                        ...
                        if ( !toWorksheet.getFormatId().equals(fromWorksheet.getFormatId())) {
                            for (i = 0; i < 30; i++ ) {
                                ...
                                if (fromName != null) {
                                    toIndex = toColumnMap.get(fromName);
                                    if (toIndex != null)
                                        toWqrVDO.setValueAt(toIndex.intValue() - 9,
                                                            fromWqrVDO.getValueAt(i));
                                }
                            }
                        } else {
                            for (i = 0; i < 30; i++ )
    ...
    public WorksheetManager1 sortItems(WorksheetManager1 wm, int col, int dir) {
        ...
        Collections.sort(WorksheetManager1Accessor.getItems(wm), new WorksheetComparator(wm,
                                                                                         col,
                                                                                         dir));
    ...
    /**
     * retrieves the sample manager and the worksheet managers associated with
    ...
    public ArrayList<Object> fetchByAccessionNumber(Integer accession) throws Exception {
        ...
        ArrayList<Object> managers;

        aids = new ArrayList<Integer>();
        managers = new ArrayList<Object>();
        wms = null;
    ...
    private void loadQcForAnalysis(WorksheetManager1 wm, TestWorksheetItemDO twiDO, int index,
                                   WorksheetAnalysisViewDO waVDO, Preferences prefs,
                                   WorksheetQcChoiceVO wqcVO,
                                   HashMap<String, ArrayList<QcLotViewDO>> qcMap) throws Exception {
        int i;
        ...
        if (qcs.size() == 0) {
            throw new FormErrorException(Messages.get()
                                                 .noMatchingActiveQc(twiDO.getQcName(),
                                                                     String.valueOf(index + 1)));
        } else if (qcs.size() > 1) {
            qcsByLoc = null;
            if (prefs != null) {
                for (QcLotViewDO tempLot : qcs) {
                    if (tempLot.getLocationId() != null &&
                        tempLot.getLocationId().equals(prefs.getInt("location", -1))) {
                        if (qcsByLoc == null)
