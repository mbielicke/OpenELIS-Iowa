...
public class SampleManagerBean {
    ...
    public SampleManager update(SampleManager man) throws Exception {
        ...
        try {
            ut.begin();
            lock.validateLock(Constants.table().SAMPLE, man.getSample().getId());
            man.update();
            lock.unlock(Constants.table().SAMPLE, man.getSample().getId());
            ut.commit();
        } catch (Exception e) {
            ut.rollback();
            throw e;
    ...
    public SampleManager fetchForUpdate(Integer id) throws Exception {
        ...
        try {
            ut.begin();
            lock.lock(Constants.table().SAMPLE, id);
    ...
    public SampleManager abortUpdate(Integer id) throws Exception {
        lock.unlock(Constants.table().SAMPLE, id);
        ...
        try {
            ...
            if (checkSample != null && !checkSample.getId().equals(data.getId())) {
                if (SampleManager.QUICK_ENTRY.equals(checkSample.getDomain())) {
                    ...
                    // since the record is a quick entry it doesn't have result
                    // records at this point.
                    // we need to add the result records for all the analyses on
                    // this sample at this point
                    for (int i = 0; i < quickEntryMan.getSampleItems().count(); i++ ) {
                        ...
                        for (int j = 0; j < anMan.count(); j++ ) {
                            ...
                            if (arMan.rowCount() <= 0) {
                                ...
                                try {
                                    arMan = AnalysisResultManager.fetchByTestId(anDO.getTestId(),
            ...
            // ignore not found because it means they entered an unused
            // accession number
        } catch (Exception e) {
