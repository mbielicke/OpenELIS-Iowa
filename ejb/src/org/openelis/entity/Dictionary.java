...
@NamedQueries({
               ...
               @NamedQuery(name = "Dictionary.FetchByCategoryIds",
                           query = "select distinct new org.openelis.domain.DictionaryViewDO(d.id,d.sortOrder,"
                                   + "d.categoryId, d.relatedEntryId, d.systemName,d.isActive,  d.code, d.entry, dre.entry, c.name)"
                                   + " from  Dictionary d left join d.relatedEntry dre left join d.category c where d.categoryId in (:ids) order by d.sortOrder "),
...
@NamedNativeQueries({
                     @NamedNativeQuery(name = "Dictionary.ReferenceCheckForId",
                                       query = "select operation_id as DICTIONARY_ID from project_parameter where operation_id = :id "
                                               ...
                                               + "UNION "
                                               + "select status_id as DICTIONARY_ID from iorder where status_id = :id "
                                               + "UNION "
                                               + "select cost_center_id as DICTIONARY_ID from iorder where cost_center_id = :id "
                                               + "UNION "
                                               + "select ship_from_id as DICTIONARY_ID from iorder where ship_from_id = :id "
                                               + "UNION "
                                               + "select container_id as DICTIONARY_ID from iorder_container where container_id = :id "
                                               + "UNION "
                                               + "select type_of_sample_id as DICTIONARY_ID from iorder_container where type_of_sample_id = :id "
                                               + "UNION "
                                               + "select type_id as DICTIONARY_ID from iorder_organization where type_id = :id "
                                               + "UNION "
                                               + "select unit_id as DICTIONARY_ID from iorder_recurrence where unit_id = :id "
                                               + "UNION "
...
@SqlResultSetMappings({
                       @SqlResultSetMapping(name = "Dictionary.ReferenceCheckForIdMapping",
    ...
    public Audit getAudit(Integer activity) {
        ...
        if (original != null)
            audit.setField("id", id, original.id)
                 ...
                 .setField("related_entry_id",
                           relatedEntryId,
                           original.relatedEntryId,
