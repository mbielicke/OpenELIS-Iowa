...
@NamedNativeQueries({
    ...
    @NamedNativeQuery(name = "Sample.FetchForClientEmailReceivedReport",
                     query = "select s.accession_number, s.collection_date, s.collection_time, s.received_date,"
                           ...
                           +       " s.status_id != (select id from dictionary where system_name = 'sample_not_verified') and"
                           +       " a.id in (select f.analysis_id from analysis_report_flags f where f.analysis_id = a.id and f.notified_received = 'N')"
                           +" UNION"
    ...
    @NamedNativeQuery(name = "Sample.FetchForClientEmailReleasedReport",
                     query = "select distinct s.accession_number, s.collection_date,s.collection_time, s.received_date, op.value email,"
                           ...
                           +       " s.status_id = (select id from dictionary where system_name = 'sample_released') and"
                           +       " a.status_id = (select id from dictionary where system_name = 'analysis_released') and"
                           ...
                           + " where a.released_date between :start_released_date and :end_released_date and"
                           +       " s.status_id in (select id from dictionary where system_name not in ('sample_released', 'sample_error')) and"
                           +       " a.status_id = (select id from dictionary where system_name = 'analysis_released') and"
                           +       " a.is_reportable = 'Y' and"
                           +       " a.id in (select f.analysis_id from analysis_report_flags f where a.id = f.analysis_id and f.notified_released = 'N')"
                           +" UNION"
                           +" select distinct s.accession_number, s.collection_date,s.collection_time, s.received_date, op.value email,"
                           +       " ss.collector ref_field1, ss.location ref_field2, CAST(s.client_reference AS varchar(20)) ref_field3,"
                           +       " CAST(p.name AS varchar(20)) ref_field4, a.id a_id"
