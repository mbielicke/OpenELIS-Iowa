...
public class AnalyteParameterManagerProxy {
    ...
    public AnalyteParameterManager fetchActiveByReferenceIdReferenceTableId(Integer referenceId,
        ...
        list = EJBFactory.getAnalyteParameter()
    ...
    public AnalyteParameterManager update(AnalyteParameterManager man) throws Exception {
        ...
        for (int i = 0; i < params.size(); i++ ) {
            ...
            if (data.getId() == null) {
                ...
                if (prevData != null && prevData.getAnalyteId().equals(data.getAnalyteId())) {
                    /*
                    ...
                    if (i == params.size() - 1) {
                        /*
                         * if this is the last active DO in the list then it
                         * could mean that the previous records for this analyte
                         * were not fetched by the screen; thus we need to find
                         * out if it has any previous records in the database so
                         * that the previous record can be updated if need be
                         */
                        if (data.isChanged())
                            updateLatestInactive(data, pl);
                    } else {
                        nextData = params.get(i + 1);
                        if ( ! (nextData.getAnalyteId().equals(data.getAnalyteId())) &&
                            data.isChanged())
                            updateLatestInactive(data, pl);
                    }
                }
    ...
    public void validate(AnalyteParameterManager man) throws Exception {
        ...
        ArrayList<AnalyteParameterViewDO> params;
        AnalyteParameterViewDO prev, data;
    ...
    private void validateDates(AnalyteParameterViewDO data, AnalyteParameterViewDO prev,
        ...
        if (data.getActiveBegin() == null || prev.getActiveBegin() == null ||
            prev.getActiveEnd() == null)
            return;
        ...
        if ( (pbt - cbt) <= 60000)
            errors.add(new FormErrorException(Messages.get()
                                                      .beginDateInvalidWithParamException(prev.getAnalyteName())));
        /*
         ...
         * the currently latest one, which could be "data", we don't check to
         * see if data's begin date is after prev's end date if both data and
         * prev are marked as active; because this means that prev was not
         * fetched to be shown on the screen and so it didn't get deactivated
         * there but will be in update()
         */
        if ( (pbt - cet) <= 0)
            errors.add(new FormErrorException(Messages.get()
                                                      .beginDateAfterPreviousEndDateException(prev.getAnalyteName())));
    }

    private boolean canAddParameter(AnalyteParameterViewDO data) {
        return (data.getActiveBegin() != null && data.getActiveEnd() != null && (data.getP1() != null ||
